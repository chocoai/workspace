...
import com.whty.wfd.common.utils.CommonFunction;
...
public class PoiExcelUtils {
	...
	private void writeExportDate(HSSFWorkbook hssfWorkbook, HSSFRow row, int cellIndex) {
		...
		cellValueHandler(cell4ExportDateValue, TimeUtils.dateTime2String(TimeUtils.getCurrentTime()));
	...
	public static HSSFWorkbook createExcelExportReport(List<Map<String, Object>> teacherUseLoglst,
			String[] teacherUseLogTableHeader, List<Map<String, Object>> deviceUseCountlst,
			String[] deviceUseCountlsttableHeader) throws Exception {

		PoiExcelUtils poiExcelUtil = new PoiExcelUtils();
		// 1.创建 Workbook
		HSSFWorkbook hssfWorkbook = poiExcelUtil.getHSSFWorkbook();
		// 2.创建 Sheet
		HSSFSheet teacherUseSheet = poiExcelUtil.getHSSFSheet(hssfWorkbook, "教师明细数据");
		HSSFSheet deviceUseSheet = poiExcelUtil.getHSSFSheet(hssfWorkbook, "设备使用统计");

		List<TeacherUseLog> teacherUseLoglst2 = new ArrayList<TeacherUseLog>();
		for (Map<String, Object> bean : teacherUseLoglst) {
			// use_taking

			TeacherUseLog teacherUseLog = new TeacherUseLog();

			teacherUseLog.setCount(bean.get("count").toString());
			teacherUseLog.setOrg_name(bean.get("org_name").toString());
			teacherUseLog.setSubject_name(bean.get("subject_name").toString());
			teacherUseLog.setUser_name(bean.get("user_name").toString());

			BigDecimal useTaking = (BigDecimal) bean.get("use_taking");
			teacherUseLog.setUse_taking(CommonFunction.formatDuring(useTaking.toString()));

			teacherUseLoglst2.add(teacherUseLog);

		}
		List<DeviceUseLog> deviceUseCountlst2 = new ArrayList<DeviceUseLog>();
		for (Map<String, Object> bean : deviceUseCountlst) {
			DeviceUseLog deviceUseLog = new DeviceUseLog();

			deviceUseLog.setClass_room_name(bean.get("class_room_name").toString());
			deviceUseLog.setName(bean.get("name").toString());
			deviceUseLog.setUse_count(bean.get("use_count").toString());

			deviceUseLog.setUser_name(bean.get("user_name").toString());
			deviceUseLog.setUser_use_count(bean.get("user_use_count").toString());

			BigDecimal useTaking = (BigDecimal) bean.get("use_taking");
			deviceUseLog.setUse_taking(CommonFunction.formatDuring(useTaking.toString()));

			deviceUseCountlst2.add(deviceUseLog);
		}

		poiExcelUtil.writeHeader(hssfWorkbook, teacherUseSheet, teacherUseLogTableHeader, 1);
		poiExcelUtil.writeContent(hssfWorkbook, teacherUseSheet, teacherUseLogTableHeader, teacherUseLoglst2, 2);

		poiExcelUtil.writeHeader(hssfWorkbook, deviceUseSheet, deviceUseCountlsttableHeader, 1);
		poiExcelUtil.writeContent(hssfWorkbook, deviceUseSheet, deviceUseCountlsttableHeader, deviceUseCountlst2, 2);
