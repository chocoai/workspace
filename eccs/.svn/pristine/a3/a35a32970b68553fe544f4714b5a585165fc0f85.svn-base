...
<a href="javascript:window.location.reload();">底稿编制列表</a>
</div>
    <div id="content">
        <form action="/step6/list.htm" method="get" id="searchForm">
          ...
          <div style="clear:both;"></div>                                           
              <table border="1" align="center" cellpadding="1" cellspacing="1" class="list_table stripe">
                   ...
                   <#list pageBean.list as step6>     
                   <tr>
	                     <td width="30" style="text-align:center;"><p>${step11_index + 1}</p></td>
	                     <td>${step6.project.no}</td>
	                     <td>${step6.project.name} </td>
	                     <td>${step6.project.recordDate}</td>
	                     <td>${step6.project.customer.cusName}</td>
	                     <td>${step6.project.serviceType.name}</td>
	                     <td>${step6.project.editorialType.name}</td>
	                     <td>${step6.project.urgentType.name}</td>
	                     <td>${step6.project.recordName}</td>
	                     <td style=" text-align:center;">
			                     <a href="/step6/edit.htm?step6.id=${step6.id}" title="编辑">编辑</a>
			                     <a href="/step6/show.htm?step6.id=${step6.id}" title="查看">查看</a>
	                     </td>                             
				</tr>
   				</#list>
   				<#else>
...
<script>
	function delstep11(id) {
		...
		var url = "/step6/delete.htm?step6.id=" + id;
		$.ajax({
			type:"post",
			url:url,
			asyn:true,
	...
	function addstep11() {
         window.parent.location.href="/step6/new.htm";
	}
</script>