...
<div id="content">
    <form action="/bid/file/list.htm" method="post" id="searchForm">
    ...
    <table border="1" align="center" cellpadding="1" cellspacing="1" class="list_table5 stripe">
        ...
        <#list pageBean.list as fileAssess>
        <tr>
            <td width="30" style="text-align: center;">${fileAssess_index+1}</td>
            <td style="text-align: center;">
                <span title="">${fileAssess.customer.cusName}</span>
            </td>
            <td style="text-align: center;">
                <span title="">${fileAssess.projectInfo.proname}</span>
            </td>
            <td style="text-align: center;">
                <span title="">${fileAssess.projectInfo.agentcompany}</span>
            </td>
            <td style="text-align: center;">
                <span title="">${fileAssess.bidOpenDate}</span>
            </td>
            <td style="text-align: center;">
                <#if fileAssess.resStates >
                <a href="javascript:;" onclick="deleteApply(${fileAssess.cid})" title="删除">删除</a>
                <a href="/bid/file/edit.htm?projectInfo.id=${fileAssess.projectInfo.id}" title="编辑">编辑 </a>
                </#if>
                <#if fileAssess.checkStates >
                <a href="javascript:;" onclick="deleteApply(${fileAssess.cid})" title="删除">删除</a>
                <a href="/bid/file/check.htm?projectInfo.id=${fileAssess.projectInfo.id}" title="审核">审核 </a>
                </#if>
                <a href="/bid/file/show.htm?projectInfo.id=${fileAssess.projectInfo.id}" title="查看">查看</a>
            </td>
        </tr>
        </#list>
    </table>
...
<script>
    function deleteApply(id) {
        ...
        var url = "/bid/file/delete.htm?fileAssess.cid=" + id;
        $.ajax({
            type : "post",
            url : url,
            asyn : true,
