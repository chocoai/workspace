...
<a href="javascript:window.location.reload();">项目总结</a>
    ...
    <div id="content">
        <form action="/step14/list.htm" method="get" id="searchForm">
          ...
          <div style="clear:both;"></div>                                           
              <table border="1" align="center" cellpadding="1" cellspacing="1" class="list_table stripe">
        		...
        		<#list pageBean.list as step14>     
                   <tr>
                     <td width="30" style="text-align:center;"><p>${step1_index + 1}</p></td>
                     <td>${step14.project.no}</td>
                     <td>${step14.project.name} </td>
                     <td>${step14.project.recordDate}</td>
                     <td>${step14.project.customer.cusName}</td>
                     <td>${step14.project.serviceType.name}</td>
                     <td>${step14.project.editorialType.name}</td>
                     <td>${step14.project.urgentType.name}</td>
                     <td>${step14.project.recordName}</td>
                     <td style=" text-align:center;">
		                     <a href="/step14/edit.htm?step14.id=${step14.id}" title="编辑">查看</a>
		                     <a href="/step14/show.htm?step14.id=${step14.id}" title="查看">查看</a>
                     </td>                             
				</tr>
		</#list>
		<#else>
...
<script>
	function delstep13(id) {
		...
		var url = "/step14/delete.htm?step14.id=" + id;
		$.ajax({
			type:"post",
			url:url,
			asyn:true,
	...
	function addstep13() {
         window.parent.location.href="/step14/new.htm";
	}
</script>
