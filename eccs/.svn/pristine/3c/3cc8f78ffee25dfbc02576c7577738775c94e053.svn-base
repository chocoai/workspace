...
<a href="javascript:window.location.reload();">征求意见稿列表</a>
</div>
    <div id="content">
        <form action="/step10/list.htm" method="get" id="searchForm">
              ...
              <table border="1" align="center" cellpadding="1" cellspacing="1" class="list_table stripe">
                 <tr class="head">
                  ...
                  <#if pageBean.list?size !=0 >
                   <#list pageBean.list as step10>     
               <tr>
                 <td width="30" style="text-align:center;"><p>${step1_index + 1}</p></td>
                 <td>${step10.project.no}</td>
                 <td>${step10.project.name} </td>
                 <td>${step10.project.recordDate}</td>
                 <td>${step10.project.customer.cusName}</td>
                 <td>${step10.project.serviceType.name}</td>
                 <td>${step10.project.editorialType.name}</td>
                 <td>${step10.project.urgentType.name}</td>
                 <td>${step10.project.recordName}</td>
                 <td style=" text-align:center;">
                         <a href="/step10/edit.htm?step10.id=${step10.id}">编辑</a>
                         <a href="/step10/show.htm?step10.id=${step10.id}">查看</a>
                 </td>                             
			</tr>
			</#list>
			<#else>
...
<script>
	function delstep10(id) {
		if (!confirm("确定删除此信息")) {
			return;
		}
		var url = "/step10/delete.htm?step10.id=" + id;
		$.ajax({
			type:"post",
			url:url,
			asyn:true,
	...
	function addstep10() {
         window.parent.location.href="/step10/new.htm";
	}
</script>
