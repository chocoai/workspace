...
<a href="javascript:window.location.reload();">资料归档列表</a>
</div>
    <div id="content">
        <form action="/step15/list.htm" method="get" id="searchForm">
          ...
          <div style="clear:both;"></div>                                           
              <table border="1" align="center" cellpadding="1" cellspacing="1" class="list_table stripe">
            		...
            		<#list pageBean.list as step15>     
                       <tr>
                         <td width="30" style="text-align:center;"><p>${step15_index + 1}</p></td>
                         <td>${step15.project.no}</td>
                         <td>${step15.project.name} </td>
                         <td>${step15.project.recordDate}</td>
                         <td>${step15.project.customer.cusName}</td>
                         <td>${step15.project.serviceType.name}</td>
                         <td>${step15.project.editorialType.name}</td>
                         <td>${step15.project.urgentType.name}</td>
                         <td>${step15.project.recordName}</td>
                         <td style=" text-align:center;">
                         		<a href="/step15/edit.htm?step15.id=${step15.id}" title="编辑">查看</a>
                        		<a href="/step15/show.htm?step15.id=${step15.id}" title="查看">查看</a>
                         </td>                             
					</tr>
   					</#list>
   					<#else>
...
<script>
	function delstep15(id) {
		if (!confirm("确定删除此信息")) {
			return;
		}
		var url = "/step15/delete.htm?step15.id=" + id;
		$.ajax({
			type:"post",
			url:url,
			asyn:true,
	...
	function addstep15() {
         window.parent.location.href="/step15/new.htm";
	}
</script>
