...
<div id="content">
	<form action="/bid/call/list.htm" method="post" id="searchForm">
	...
	<table border="1" align="center" cellpadding="1" cellspacing="1" class="list_table5 stripe">
		...
		<#list pageBean.list as callBidAssess>
        <tr>
            <td width="30" style="text-align: center;">${callBidAssess_index+1}</td>
            <td style="text-align: center;">
                <span title="">${callBidAssess.projectInfo.bidmen.cusName}</span>
            </td>
            <td style="text-align: center;">
                <span title="">${callBidAssess.projectInfo.proname}</span>
            </td>
            <td style="text-align: center;">
                <span title="">${callBidAssess.projectInfo.agentcompany}</span>
            </td>
            <td style="text-align: center;">
                <span title="">${callBidAssess.projectInfo.opentime}</span>
            </td>
            <td style="text-align: center;">
                <#if callBidAssess.resStates >
                <a href="javascript:;" onclick="deleteApply(${callBidAssess.cid})" title="删除">删除</a>
                <a href="/bid/call/edit.htm?projectInfo.id=${callBidAssess.projectInfo.id}" title="编辑">编辑 </a>
                </#if>
                <a href="/bid/call/show.htm?projectInfo.id=${callBidAssess.projectInfo.id}" title="查看">查看</a>
            </td>
        </tr>
        </#list>
	</table>
...
<script>
    function deleteApply(id) {
        ...
        var url = "/bid/call/delete.htm?callAssess.cid=" + id;
        $.ajax({
            type : "post",
            url : url,
            asyn : true,
