...
<div id="content">
	<form action="/bid/transfer/list.htm" method="post" id="searchForm">
	...
	<table border="1" align="center" cellpadding="1" cellspacing="1" class="list_table5 stripe">
		...
		<#list pageBean.list as transfer>
		<tr>
			<td width="30" style="text-align: center;">${transfer_index+1}</td>
			<td style="text-align: center;">
				<span >${transfer.projectInfo.bidmen.cusName}</span>
			</td>
			<td style="text-align: center;">
				<span>${transfer.projectInfo.proname}</span>
			</td>
			<td style="text-align: center;">
				<span>${transfer.projectInfo.agentcompany}</span>
			</td>
			<td style="text-align: center;">
				<span>${transfer.projectInfo.opentime}</span>
			</td>
			<td style="text-align: center;">
				<#if transfer.resStates >
                <a href="javascript:;" onclick="deleteApply(${transfer.cid})" title="删除">删除</a>
				<a href="/bid/transfer/edit.htm?projectInfo.id=${transfer.projectInfo.id}" title="编辑">编辑 </a>
				</#if>
				<a href="/bid/transfer/show.htm?projectInfo.id=${transfer.projectInfo.id}" title="查看">查看</a>
			</td>
		</tr>
		</#list>
	</table>
...
<script>
    function deleteApply(id) {
        ...
        var url = "/bid/transfer/delete.htm?transfer.cid=" + id;
        $.ajax({
            type : "post",
            url : url,
            asyn : true,
