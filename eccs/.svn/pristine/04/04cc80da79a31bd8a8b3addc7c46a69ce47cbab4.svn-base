...
<div id="content">
    <form action="/bid/info/list.htm" method="post" id="searchForm">
    ...
    <table border="1" align="center" cellpadding="1" cellspacing="1" class="list_table5 stripe">
        ...
        <#list pageBean.list as info>
        <tr>
            <td width="30" style="text-align: center;">${info_index+1}</td>
            <td style="text-align: center;">
                <span title="">${info.projectInfo.bidmen.cusName}</span>
            </td>
            <td style="text-align: center;">
                <span title="">${info.projectInfo.proname}</span>
            </td>
            <td style="text-align: center;">
                <span title="">${info.projectInfo.agentcompany}</span>
            </td>
            <td style="text-align: center;">
                <span title="">${info.bidOpenDate}</span>
            </td>
            <td style="text-align: center;">
                <span title="">
					<#if info.isSuccess??>
						${(info.isSuccess==1)?string("中标","未中标")}
					<#else>
						待开标
					</#if>
				</span>
            ...
            <td style="text-align: center;">
                <#if info.resStates >
                <a href="javascript:;" onclick="deleteApply(${info.cid})" title="删除">删除</a>
                <a href="/bid/info/edit.htm?projectInfo.id=${info.projectInfo.id}" title="编辑">编辑 </a>
                </#if>
                <a href="/bid/info/show.htm?projectInfo.id=${info.projectInfo.id}" title="查看">查看</a>
            </td>
        </tr>
        </#list>
    </table>
...
<script>
    function deleteApply(id) {
        ...
        var url = "/bid/info/delete.htm?info.cid=" + id;
        $.ajax({
            type : "post",
            url : url,
            asyn : true,
