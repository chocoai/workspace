...
<a href="javascript:window.location.reload();">校对列表</a>
</div>
    <div id="content">
        <form action="/step7/list.htm" method="get" id="searchForm">
        ...
        <#list pageBean.list as step7>     
                   <tr>
	                     <td width="30" style="text-align:center;"><p>${step1_index + 1}</p></td>
	                     <td>${step7.project.no}</td>
	                     <td>${step7.project.name} </td>
	                     <td>${step7.project.recordDate}</td>
	                     <td>${step7.project.customer.cusName}</td>
	                     <td>${step7.project.serviceType.name}</td>
	                     <td>${step7.project.editorialType.name}</td>
	                     <td>${step7.project.urgentType.name}</td>
	                     <td>${step7.project.recordName}</td>
	                     <td style=" text-align:center;">
			                     <a href="/step7/edit.htm?step7.id=${step7.id}" title="编辑">编辑</a>
			                     <a href="/step7/show.htm?step7.id=${step7.id}" title="查看">查看</a>
	                     </td>                             
				</tr>
   				</#list>
   				<#else>
...
<script>
	function delStep7(id) {
		if (!confirm("确定删除此信息")) {
			return;
		}
		var url = "/step7/delete.htm?step7.id=" + id;
		$.ajax({
			type:"post",
			url:url,
			asyn:true,
	...
	function addStep7() {
         window.parent.location.href="/step7/new.htm";
	}
</script>