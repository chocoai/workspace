...
<script type="text/javascript">
    if("${fileAssess.completeStatus}"==1){
        ...
        $(":radio").eq(0).remove("checked","checked");
    }
	...
	function toNextStep() { //执行下一步
		...
		var message = "";
		if(operaterType==0){
			message = "当前步骤执行完毕,下一环节为【投标文件审核】，下一环节处理人为【" + nextOperator + "】，确定要执行下一步吗?";
		}else{
			message = "确认终止此项目吗？";
		}
		if (confirm(message)) {
			document.getElementById("contractForm").action = "/bid/file/toNextStep.htm";
			$("#contractForm").submit();
		}
	}
