...
//                var eD = new Date(events[j][3]);
        ...
        function getTitle(event) {
            ...
            eventshow = event[1];
            var startformat = getymformat(new Date(event[2]), null, showtime, true);
            var endformat = getymformat(new Date(event[3]), new Date(event[2]), showtime, true);
            ...
            if (option.url && option.url != "") {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                    	...
                    	for ( var i = 0; i < events.length; i++) {
                    		data.events[i][2] = new Date(data.events[i][2]);
                    		data.events[i][3] = new Date(data.events[i][3]);
						}
                    	//function(datastr) {									
