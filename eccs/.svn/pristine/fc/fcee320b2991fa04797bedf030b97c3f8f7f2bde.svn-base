...
<a href="javascript:window.location.reload();">审核</a>
  ...
  <#list step8ItemList as step8Item>
  		<tr>
		    <td style=" text-align:center;background-color:#fff;">${step8Item.content}</td>
  ...
  <tr>
          ...
          <td  style=" background-color:#fff;"> ${step8.validateName}</td>
          <td  class="tab_title">审核日期</td>     
          <td  style="background-color:#fff;"> ${step8.validateDate}</td>       
  </tr>
  <tr>
...
<script>
	function toPrint(id){
		var url = "/step8/print.htm?project.id=" + id;
