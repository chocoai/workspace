...
<a href="javascript:window.location.reload();">登记资料交接列表</a>
</div>
    <div id="content">
        <form action="/step2/list.htm" method="get" id="searchForm">
          ...
          <div style="clear:both;"></div>                                           
            ...
            <#list pageBean.list as step2>     
                       <tr>
	                         <td width="30" style="text-align:center;"><p>${step2_index + 1}</p></td>
	                         <td>${step2.project.no}</td>
	                         <td>${step2.project.name} </td>
	                         <td>${step2.project.recordDate}</td>
	                         <td>${step2.project.customer.cusName}</td>
	                         <td>${step2.project.serviceType.name}</td>
	                         <td>${step2.project.editorialType.name}</td>
	                         <td>${step2.project.urgentType.name}</td>
	                         <td>${step2.project.recordName}</td>
	                         <td style=" text-align:center;">
		                        	 <a href="/step2/edit.htm?step2.id=${step2.id}" title="编辑">查看</a>
		                         	<a href="/step2/show.htm?step2.id=${step2.id}" title="查看">查看</a>
	                         </td>                             
					</tr>
   				</#list>
   				<#else>
...
<script>
	function delStep2(id) {
		if (!confirm("确定删除此信息")) {
			return;
		}
		var url = "/step2/delete.htm?step2.id=" + id;
		$.ajax({
			type:"post",
			url:url,
			asyn:true,
	...
	function addStep2() {
         window.parent.location.href="/step2/new.htm";
	}
</script>