...
public class ReflectionUtil {
	...
	public static Object invokeMethod(final Object target,
			...
			final Object... args) {
		if (CollectionUtil.notEmpty(paramTypes)
		...
		try {
			...
			log.error("无此方法：" + methodName + "("
					+ (msg.length() > 0 ? msg.substring(2) : msg) + ")");
	 ...
	 * @return
	 */
	public static Field getDeclaredField(final Object target,
			final String fieldName) {
		Assert.notNull(target);
		try {
	...
	public static Field getDeclaredField(final Class<?> clazz,
		...
		throw new NoSuchFieldException(
				"No such field: " + clazz.getName() + '.' + fieldName);
	}
	 ...
	 */
	public static void bean2Bean(final Object dest, final Object orig,
			String except) {
		Field[] fieldArr = dest.getClass().getDeclaredFields();
		String[] excludes = StringUtil.trimAllWhitespace(except).split(",");
		for (Field field : fieldArr) {
			String fieldName = field.getName();
			Object value = getFieldValue(orig, fieldName);
			if (ArrayUtils.contains(excludes, fieldName)) {
				continue;
			}
			setFieldValue(dest, fieldName, value);
		}
	}
