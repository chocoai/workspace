...
<div id="content">
	<form action="/bid/summary/list.htm" method="post" id="searchForm">
	...
	<table border="1" align="center" cellpadding="1" cellspacing="1" class="list_table5 stripe">
		...
		<#list pageBean.list as summary>
		<tr>
			<td width="30" style="text-align: center;">${summary_index+1}</td>
			<td>
				<span title="">${summary.projectInfo.bidmen.cusName}</span>
			</td>
			<td>
				<span title="">${summary.projectInfo.proname}</span>
			</td>
			<td>
				<span title="">${summary.projectInfo.agentcompany}</span>
			</td>
			<td>
			    <span title="">${summary.projectInfo.opentime}</span>
			</td>
			<td style="text-align: center;">
                <!-- <a href="javascript:;" onclick="deleteApply(${summary.cid})" title="删除">删除</a> -->
				<a href="/bid/summary/edit.htm?projectInfo.id=${summary.projectInfo.id}" title="编辑">编辑 </a>
				<a href="/bid/summary/show.htm?projectInfo.id=${summary.projectInfo.id}" title="查看">查看</a>
			</td>
		</tr>
		</#list>
	</table>
...
<script>
    function deleteApply(id) {
        ...
        var url = "/bid/summary/delete.htm?summary.cid=" + id;
        $.ajax({
            type : "post",
            url : url,
            asyn : true,
