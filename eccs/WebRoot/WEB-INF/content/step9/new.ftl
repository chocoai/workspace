...
<a href="javascript:window.location.reload();">审定</a>
  ...
  <table border="1" cellspacing="1" cellpadding="1" class="input_table">
	  ...
	  <tr>
		    ...
		    <td style="background-color:#fff;">
				<input name="step9.id" value="${step9.id}"  type="hidden" />
  	 ...
  	 <tr>
    		<td colspan="4"  >
	              ...
	              <a href="javascript:addstep9Item(); " >+新增</a>             
	     		  </div>
	    <table border="1" id="step9ItemId" cellspacing="1" cellpadding="1" class="list_table3" style=" margin-top:10px;">
  		...
  		<#list step9ItemList as step9Item>
  			<tr>
			    <td style=" text-align:center;background-color:#fff;"><input type="text" name="content" value="${step9Item.content}"  class="text_a"  /><input type="hidden" name="values" value='${step9Item_index}'  class="text_a"  /></td>
			    <td style=" text-align:center;background-color:#fff;"><input name="result_${step9Item_index}"  type="radio" value="1"  <#if step9Item.result == 1>checked="checked"</#if>   />&nbsp;修改通过
                    &nbsp;&nbsp;&nbsp;<input name="result_${ step9Item_index}"  type="radio" value="2"   <#if step9Item.result == 2>checked="checked" </#if> />&nbsp;修改未通过  
    				&nbsp;&nbsp;&nbsp;<input name="result_${ step9Item_index}"  type="radio" value="0"   <#if step9Item.result == 0>checked="checked" </#if> />&nbsp;未修改  </td>
     			<td style=" text-align:center;"><a href="javascript:deletesz(${step9Item.id})">删除</a></td>
  			</tr>
  		</#list>
	</table>
    </td>
  ...
  <tr>
          ...
          <td   style="background-color:#fff;" >
          <input name="step9.writeName" value="${step9.writeName}"  type="hidden"/>${step6.user.name}</td>
          <td  class="tab_title">编制日期</td>     
          <td   style="background-color:#fff;"> <input value="${step6.confirmTime}" type="hidden" />${step6.confirmTime}</td>       
  </tr>
  <tr>
          <td  class="tab_title red"> 审定人</td>
          <td  class="tab_title">
          		<#if step9.validateName!=null>
	          			<input name="step9.validateName" value="${step9.validateName}"  type="text" class="text_a" />
	           <#else>
	           			<input name="step9.validateName" value="${user.name}"   type="text" class="text_a" />
	           </#if>
           </td>
          <td  class="tab_title">审定日期</td>     
          <td   style="background-color:#fff;">
          		<#if step9.validateDate!=null>
	          			<input name="step9.validateDate" value="${step9validateDate}" type="hidden" />${step9.validateDate}
	           <#else>
	           			<input name="step9.validateDate" value="${validateDate}" type="hidden" />${validateDate}
	           </#if> 
          </td>       
  </tr>
  <tr>
...
<script>
	...
	$().ready(function() {  
	  	      $("#contractForm").validate({  
					rules : {  
				 'step9.writeName' : { required : true },
		         'step9.validateName' : { required : true },
		         'step9.validateDate' : { required : true }
							}
					})
	}); 
	...
	function returnParam2(id,name,usernmae) {
		$('#nextWorkerId').val(id);
		document.getElementById("contractForm").action="/step9/save.htm";
	...
	function AddRow()
	{
		...
		var tab1 = document.getElementById("step9ItemId");
