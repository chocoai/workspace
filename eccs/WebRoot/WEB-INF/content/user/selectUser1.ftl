...
<html>
	...
	<form action="/user/selectUser1.htm" method="post" id="searchForm">
		<div>
			<input name="name1" id="name" placeholder="请输入用户名" value="${name1}"
				type="text"
			...
			<input type="hidden" name="ids" id="ids" value="${values}" />
		</div>
<input type="button" class="btn" onclick="javascript:$('#searchForm').submit();" value="查询" style="float: left; line-height: 22px; width: 50px; height: 30px; text-indent: 6px; margin: 6px 20px 0px 20px;"	size="100px"/>
			...
			<div id="content">
				<table width="90%" border="1" align="center" cellpadding="1"
					...
					<tr>
						...
						<td style="text-align: center;background-color:#fff;">
							<!--<a href="javascript:addUser('${user.id}', '${user.name}')">选择</a>-->
							<input type="checkbox" name="checkbox" value="${user.id}"
								onclick="javascript:addvalues(this);" />
						</td>
					</tr>
					</#list>
				</table>
	...
	<script>
		...
		$(function() {
			var objid = window.dialogArguments.ids;
		...
		function addvalues(o) {
			...
			$.ajax( {
				type : "get",
				url : '/user/selectUser4.htm?type=0&values=' + ids + '&start=' + start,
		...
		function addUser() {
			var ids = $("#ids").val();
			window.opener.returnParam1(ids);
			window.close();
		}
