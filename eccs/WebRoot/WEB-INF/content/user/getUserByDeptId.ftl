...
<form action="/user/getUserByDeptId.htm" method="post" id="searchForm">
	<div
	...
	>用户名</div>
	<input name="name1" id="name" value="${name1}" type="text"
...
<script>
	function addUser(userId, name, userName) {
		...
		window.opener.document.getElementById(elementUserId).value = userId;
		window.opener.document.getElementById(elementUserNameId).value = name;
		window.close();
	}
</script>
</html>
