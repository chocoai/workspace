...
<a href="javascript:window.location.reload();">咨询任务书列表</a>
</div>
    <div id="content">
        <form action="/step1/list.htm" method="get" id="searchForm">
          ...
          <div style="clear:both;"></div>                                           
              <table border="1" align="center" cellpadding="1" cellspacing="1" class="list_table stripe">
                            ...
                            <#list pageBean.list as step1>     
                                       <tr>
                                         <td width="30" style="text-align:center;"><p>${step1_index + 1}</p></td>
                                         <td>${step1.project.no}</td>
                                         <td>${step1.project.name} </td>
                                         <td>${step1.project.recordDate}</td>
                                         <td>${step1.project.customer.cusName}</td>
                                         <td>${step1.project.serviceType.name}</td>
                                         <td>${step1.project.editorialType.name}</td>
                                         <td>${step1.project.urgentType.name}</td>
                                         <td>${step1.project.recordName}</td>
                                         <td style=" text-align:center;">
                                         		<a href="/step1/edit.htm?step1.id=${step1.id}" title="编辑">编辑</a>
                                         		<a href="/step1/show.htm?step1.id=${step1.id}" title="查看">查看</a>
                                         </td>                             
                					</tr>
   							</#list>
   							<#else>
...
<script>
	function delStep1(id) {
		...
		var url = "/step1/delete.htm?step1.id=" + id;
		$.ajax({
			type:"post",
			url:url,
			asyn:true,
	...
	function addStep1() {
         window.parent.location.href="/step1/new.htm";   
	}
</script>
