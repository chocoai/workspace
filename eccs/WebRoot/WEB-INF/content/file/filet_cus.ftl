...
<script >
  function uploadFile(){
      var wl= window.dialogArguments;
      var annexTypeId = $("#annexTypeId").val();
      var fileNum = $("#fileNum").val();
      var fileOwner = $("#fileOwner").val();
      var filePage = $("#filePage").val();
      var description = $("#description").val();
      window.returnValue =$("#file1").val();
      ajaxFileUpload(wl,annexTypeId,annexTypeId,fileNum,fileOwner,filePage,description);
  }
  function ajaxFileUpload(wl,annexTypeId,fileNum,fileOwner,filePage,description) {
      $.ajaxFileUpload
      (
          {
              url: '/t_customer/uploadFile.htm?project.id=${projectId}&annexTypeId='+annexTypeId+'&fileNum='+fileNum+'&fileOwner='
