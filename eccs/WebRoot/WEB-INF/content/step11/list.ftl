...
<a href="javascript:window.location.reload();">成果文件编制列表</a>
</div>
    <div id="content">
        <form action="/step11/list.htm" method="get" id="searchForm">
          ...
          <div style="clear:both;"></div>                                           
              <table border="1" align="center" cellpadding="1" cellspacing="1" class="list_table stripe">
                 ...
                 <#if pageBean.list?size !=0 >
       			 <#list pageBean.list as step11>     
                   <tr>
	                     <td width="30" style="text-align:center;"><p>${step11_index + 1}</p></td>
	                     <td>${step11.project.no}</td>
	                     <td>${step11.project.name} </td>
	                     <td>${step11.project.recordDate}</td>
	                     <td>${step11.project.customer.cusName}</td>
	                     <td>${step11.project.serviceType.name}</td>
	                     <td>${step11.project.editorialType.name}</td>
	                     <td>${step11.project.urgentType.name}</td>
	                     <td>${step11.project.recordName}</td>
	                     <td style=" text-align:center;">
			                     <a href="/step11/edit.htm?step11.id=${step11.id}" title="编辑">编辑</a>
			                     <a href="/step11/show.htm?step11.id=${step11.id}" title="查看">查看</a>
	                     </td>                             
				</tr>
				</#list>
				<#else>
...
<script>
	function delstep11(id) {
		if (!confirm("确定删除此信息")) {
			return;
		}
		var url = "/step11/delete.htm?step11.id=" + id;
		$.ajax({
			type:"post",
			url:url,
			asyn:true,
	...
	function addstep11() {
         window.parent.location.href="/step11/new.htm";
	}
</script>
