...
<div id="content">
	<div style="width: 90%; min-width: 980px; margin: 20px auto;">
		<div class="tabPanel">
			<ul>
				<li class="hit" style="font-size: 14px;" onclick="location.href = '/backlog/list.htm';">待办事项</li>
				<li style="font-size: 14px;" onclick="javascript:getMyFinishedBacklog();">已办事项</li>
				<li style="font-size: 14px;" onclick="javascript:getAllFinishedBacklog();">所有事项</li>
			</ul>
			<div class="panes">
				<div class="pane" style="display: block;">
					<form action="/backlog/list.htm" method="post" id="searchForm">
					...
					<table border="1" align="center" cellpadding="1" cellspacing="1" class="list_table stripe">
						...
						<#list pageBean.list as processState>
						<tr>
							<td style="text-align: center;">
								<p>${processState_index + 1}</p>
							</td>
							<td>${processState.project.no}<#if processState.project.no == null>${processState.projectInfo.proNo}</#if></td>
							<td style="white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">
								<span title="${processState.project.name}">${processState.project.name}<#if processState.project.name ==null>${processState.projectInfo.proname}</#if></span>
							</td>
							<td style="white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">
								<span title="${processState.project.editorialType.name}">${processState.project.editorialType.name}</span>
							</td>
							<td>${processState.project.urgentType.name}</td>
							<td>${processState.project.recordName}</td>
							<td>${processState.currentStep.stepName}</td>
							<td style="text-align: center;">
							    <#if processState.currentStep.stepCode?length gt 1&&(processState.currentStep.stepCode?substring(0,2) = '-4'||processState.currentStep.stepCode = '-5'||processState.currentStep.stepCode = '-3')>
								    <a href="${processState.currentStep.url}?projectInfo.id=${processState.projectInfo.id}">处理</a>
							    <#else>
                                    <a href="${processState.currentStep.url}?project.id=${processState.project.id}">处理</a>
							    </#if>
							</td>
						</tr>
						</#list>
