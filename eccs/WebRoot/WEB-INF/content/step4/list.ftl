...
<a href="javascript:window.location.reload();">整理资料清单列表</a>
</div>
    <div id="content">
        <form action="/step4/list.htm" method="get" id="searchForm">
        ...
        <#list pageBean.list as step4>     
                   <tr>
	                     <td width="30" style="text-align:center;"><p>${step4_index + 1}</p></td>
	                     <td>${step4.project.no}</td>
	                     <td>${step4.project.name} </td>
	                     <td>${step4.project.recordDate}</td>
	                     <td>${step4.project.customer.cusName}</td>
	                     <td>${step4.project.serviceType.name}</td>
	                     <td>${step4.project.editorialType.name}</td>
	                     <td>${step4.project.urgentType.name}</td>
	                     <td>${step4.project.recordName}</td>
                    	 <td style=" text-align:center;">
			                     <a href="/step4/edit.htm?step4.id=${step4.id}" title="编辑">查看</a>
			                     <a href="/step4/show.htm?step4.id=${step4.id}" title="查看">查看</a>
                     	</td>                             
				</tr>
   				</#list>
   				<#else>
...
<script>
	function delstep4(id) {
		if (!confirm("确定删除此信息")) {
			return;
		}
		var url = "/step4/delete.htm?step4.id=" + id;
		$.ajax({
			type:"post",
			url:url,
			asyn:true,
	...
	function addstep4() {
         window.parent.location.href="/step4/new.htm";
	}
</script>
