			...
			<div class="panes">
				<div class="pane" style="display: block;">
					<form action="/project/myProjectList.htm" method="post" id="searchForm">
						<div align="center" class="list_table stripe" style="width: 90%; margin: 0 auto; min-width: 980px;">
							<div class="article" style="margin: 0; clear: both; width: 100%;">
								...
								<div class="sub_sear" onclick="javascript:getMyProjectDone();">查询</div>
					...
					<table border="1" align="center" cellpadding="1" cellspacing="1" class="list_table stripe">
						...
						<tr onclick="javascript:project('/project/showmyproject.htm?project.id=${project.id}')">
							<td style="text-align: center;">
								<input type="checkbox" id="checked" name="checkbox" value="${project.id}" onclick="addvalues(this,event);" />
							</td>
							<td style="text-align: center;">
								<p>${project_index + 1}</p>
							</td>
							<td style="white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">
								<span title="${project.no}"> ${project.no} </span>
							</td>
							<td style="white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">
								<span title="${project.name}"> ${project.name} </span>
							</td>
							<td style="text-align: center;">${project.urgentType.name}</td>
							...
							<td style="text-align: center;">
								<a href="/backlog/show.htm?project.id=${project.id}">处理详情</a>
							</td>
						</tr>
						</#list>
					</table>
				</div>
				<div style="width: 90%; margin: -40px auto; min-width: 980px; margin-top: -40px;"><#import
					"/WEB-INF/tld/page.ftl" as tt> <@tt.page pageBean=pageBean formId="searchForm" /></div>
			</div>
		</div>
	</div>
</div>
<script>
	...
	function returnParam3(value) {
		...
		if (arr[0] != null && arr[0] != "") {//arr[0]用户id arr[1]项目id
			$.ajax({
				type : "post",
				url : '/project/saveShare.htm?usersid=' + arr[0] + '&shareid='
						+ arr[1],
				asyn : true,
				dataType : 'text',
				success : function(d) {
