$(document).ready(function() {
			...
			success : function(data) {
				...
				if(data.list.length>0){
					for (var i = 0; i < data.list.length; i++) {
						...
						list += '<option value="'+bean.cid+'" unit_index="'+bean.unitCid+'"  title="性别:'+bean.sexStr+',出生日期:'+bean.birthday+'" class="athlete" id="athlete_'+bean.cid+'">'
						if(bean.alias){
	...
	$("#submitBtn").click(function(){
		...
		}else if(isTeam==0||(isTeam==1&isVilidataTeam(projectCid,athleteCids))){
	...
	$.ajax({
		...
		success : function(data) {
			...
			if(data.states=='noathlete'){
				...
				layer.msg('参与该项目男性运动员出生范围：'+data.param);
			}else if(data.states=='groupWomanAgeStart'){
				layer.msg('参与该项目女性运动员出生范围：'+data.param);
	...
	$.ajax({
		...
		success : function(data) {
			...
			if(data.states=='noathlete'){
				...
				layer.msg('该团体项目需【'+data.param+'】名运动员上场参赛');
