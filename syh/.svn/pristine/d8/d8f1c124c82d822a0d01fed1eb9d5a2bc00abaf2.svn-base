		...
		$.ajax({
			...
			success : function(data) {
				...
				for (var i = 0; i <data.unit.length; i++) {
					...
					tr4 += "<th>"+isStringNUll(bean.gold)+"</th><th>"+isStringNUll(bean.silver)+"</th><th>"+isStringNUll(bean.copper)+"</th><th>"+isStringNUll(bean.intrgral)+"</th><th>"+isStringNUll(bean.total)+"</th>"
				}
				tr1 += "</tr>";
				tr2 += "</tr>";
				tr4 += "</tr>";
				...
				for (var k = 0; k <data.rows.length; k++) {
					...
					$("#"+goldid).text(isStringNUll(bean.gold));
					$("#"+silverid).text(isStringNUll(bean.silver));
					$("#"+copperid).text(isStringNUll(bean.copper));
					$("#"+intrgralid).text(isStringNUll(bean.intrgral));
					$("#"+totalid).text(isStringNUll(bean.total));
				}
