$(function() {
	$("#refreshTable").click(function(){
		...
		$.ajax({
			type : 'POST',
			url : $("#contextPath").val() + '/warnInfo/toPersonNumPage',
			data : {
				...
				if(data.pageInfo.pages>0){
					for (var i = 0; i < data.pageInfo.list.length; i++) {
						...
						warnInfoList+='<tr>'+
							...
							'<td>'+isStringNUll(bean.personTotnum)+"/"+isStringNUll(bean.cPersonTotnum)+'</td>'+
							'<td>'+isStringNUll(bean.personSinglenum)+"/"+isStringNUll(bean.cPersonSinglenum)+'</td>'+
							'<td>'+isStringNUll(bean.personTeamnum)+"/"+isStringNUll(bean.cPersonTeamnum)+'</td>'+
							'</tr>'
					}
					$("#warnInfoList").html(warnInfoList);
