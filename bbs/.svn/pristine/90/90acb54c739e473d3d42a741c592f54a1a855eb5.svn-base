...
K.options = {
	...
	items : [
		...
		'justifyfull', 'indent', 'outdent', 'quickformat', '|',
		'formatblock', 'fontname', 'fontsize', '|', 'forecolor', 'hilitecolor', 'bold',
		'italic', 'underline', 'lineheight', '|', 'image', 'hr', 'emoticons'
	],
...
function _formatUrl(url, mode, host, pathname) {
	...
	if (mode === 'relative') {
		...
		if (url.substr(0, host.length) === host) {
			//url = url.substr(host.length);//对我返回的图片路径进行拆分了，返回的是图片的相对路径，导致项目外图片访问不到
		}
	}
	return url;
}
