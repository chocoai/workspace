 ...
 * TOM
 */
package com.yhcrt.weihu.bbs.webservices;
...
public class UserService extends SpringBeanAutowiringSupport {
    ...
    public String addUser(String auth_username, String auth_password, String username,
	...
	if (validate(auth_username, auth_password)) {
		...
		try {
		    bbsUserMng.registerMember(username, email, false, password, LOCAL_IP,
			    group.getId(), userExt, null);
	...
	if (validate(auth_username, auth_password)) {
		...
		if (user != null) {
			...
			bbsUserMng.updateMember(user.getId(), email, password, realname, sexBoolean,
				tel);
			responseCode = RESPONSE_CODE_SUCCESS;
