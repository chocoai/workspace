...
import com.yhcrt.weihu.bbs.dao.BbsMessageDao;
...
public class BbsMessageDaoImpl extends HibernateBaseDao<BbsMessage, Integer>
	...
	public List<BbsMessage> getListByUserIdStatus(Integer userId, Integer typeId,
			Boolean status) {
	...
	public Pagination getPagination(Boolean sys,Integer pageNo,Integer pageSize){
		...
		if(sys!=null){
			if(sys){
				f.append("and bean.sys=true ");
			}else{
				f.append("and bean.sys=false ");
			}
		}
