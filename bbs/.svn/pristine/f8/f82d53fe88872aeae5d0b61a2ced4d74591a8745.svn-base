...
import com.yhcrt.weihu.bbs.dao.BbsUserOnlineDao;
...
public class BbsUserOnlineMngImpl implements BbsUserOnlineMng {
	...
	public void updateUserOnlineTime(BbsSession userSession){
		Calendar curr = Calendar.getInstance();
		Calendar last = Calendar.getInstance();
		Date startActiveTime=userSession.getFirstActivetime();
		Double timegap=(userSession.getLastActivetime().getTime()-startActiveTime.getTime())/(1000.0*60*60);
		int currDay = curr.get(Calendar.DAY_OF_YEAR);
		int lastDay = last.get(Calendar.DAY_OF_YEAR);
		int currYear = curr.get(Calendar.YEAR);
		int lastYear = last.get(Calendar.YEAR);
