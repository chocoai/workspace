...
import com.yhcrt.weihu.bbs.action.member.ImageCutAct;
...
import com.yhcrt.weihu.common.util.PropertiesUtil;
...
public class ImageCutAct {
	...
	public String imageAreaSelect(String uploadBase, String imgSrcPath,
		...
		model.addAttribute("imgSrcPath", imgSrcPath.replaceAll("：", ":"));
		...
		try {
			if (imgWidth > 0) {
				if (site.getConfig().getUploadToDb()) {
					...
					String pathUrl = PropertiesUtil.getPropertyValue("bbs.properties", "attach.dir");
					String ip = PropertiesUtil.getPropertyValue("bbs.properties", "attach.http");
					String imgPath3 = pathUrl + imgSrcPath.replaceAll(ip, "").replaceAll("/", "\\\\");
					
					File file = fileRepository.retrieve(imgPath3);
					imageScale.resizeFix(file, file, reMinWidth, reMinHeight,
							getLen(imgTop, imgScale),
							getLen(imgLeft, imgScale), getLen(imgWidth,
									imgScale), getLen(imgHeight, imgScale));
