...
import com.yhcrt.weihu.bbs.manager.BbsMemberMagicMng;
import com.yhcrt.weihu.bbs.manager.BbsPointDetailMng;
import com.yhcrt.weihu.bbs.manager.BbsUserActiveLevelMng;
...
public class BbsUserMngImpl implements BbsUserMng {
	...
	public List<BbsUser> getHostUser(Integer count) {
		return dao.getHostUser(count);
	}

	@Override
	public Page<BbsUser> searchUser(Integer pageSize, Integer pageNo, String keywords) {
		return dao.searchUser(pageSize, pageNo, keywords);
	}
	
	@Override
	public BbsUser updateImg(Integer userId, String avatar) {
		BbsUser newUser = dao.findById(userId);
		if(avatar != null){
			newUser.setAvatar(avatar);
		}
		//上传图片完毕后加积分    永久就一次
		//判断是否是第一次上传（即之前是否加过积分）
		boolean isUploadImg = bbsPointDetailMng.isUploadImg(userId);
		if(!isUploadImg){
			String description =  "<b>"+newUser.getUsername()+"</b>完成<b>头像上传</b>任务,获得系统赠送积分";;
			bbsPointDetailMng.save(userId, 10, description, Constants.POINT_IMG, Constants.POINT_TYPE_IMG);
			newUser.setPoint(newUser.getPoint()+10);
			List<BbsUserGroup> list = bbsUserGroupMng.getListOfPersonal();
			Integer pointsAll = bbsPointDetailMng.getPointsAll(newUser.getId());
			for(int i=0; i<list.size(); i++){
				if(pointsAll>=list.get(i).getPoint()){
					continue;
				}else if(i == list.size()-1){
					if(newUser.getGroup().getId()!=list.get(i).getId()){
						updateGroup(newUser.getId(), list.get(i).getId());
						break;
					}else{
						break;
					}
				}else{
					if(newUser.getGroup().getId()!=list.get(i-1).getId()){
						updateGroup(newUser.getId(), list.get(i-1).getId());
						break;
					}else{
						break;
					}
