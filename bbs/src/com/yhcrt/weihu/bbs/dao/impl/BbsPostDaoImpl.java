...
import com.yhcrt.weihu.bbs.dao.BbsPostDao;
...
public class BbsPostDaoImpl extends HibernateBaseDao<BbsPost, Integer>
	...
	public Page<BbsPost> getPage(Integer userId, Integer pageSize, Integer pageNo) {
		Page<BbsPost> page = new Page<BbsPost>(pageNo,pageSize);
		...
		String hql = "from BbsPost p where p.creater.id="+userId+" and p.indexCount>1 and p.status>=0 group by p.topic.id ";
		String hqlCount = "select count(*) from BbsPost p where p.creater.id="+userId+" and p.indexCount>1 and p.status>=0 group by p.topic.id ";
		List<Object> listCount = getSession().createQuery(hqlCount).list();
		if(listCount == null){
			page.setList(list);
			page.setTotalCount(0);
			return page;
		}else{
			page.setTotalCount(listCount.size());
		}
	...
	public Pagination getForTag(Integer siteId, Integer topicId,
		...
		if (topicId != null) {
			...
			if(userId == null){
				f.append(" and bean.indexCount>1");
			}
		}
		if (userId != null) {
