...
globalObject.openTab = function(tabId, tabTitle, tab, config) {
	// console.log(config);
	if(tab == ''){
		globalObject.msgTip(tabTitle+'暂未开发！');
		return ;
	}
	...
	if (!_tab) {
		...
		_tab = Ext.create('Ext.panel.Panel', {
			...
			autoScroll : true,
			border : false,
			items : typeof (tab) == 'string' ? Ext.create('Yhcrt.app.' + tab, config) : tab
		});
		mainTab.add(_tab);
...
globalObject.openWindow = function(winTitle, win, winWidth, config) {
	Ext.create('Ext.window.Window', {
		...
		resizable : false,
		width : winWidth || 300,
		items : typeof (win) == 'string' ? Ext.create('Yhcrt.app.' + win, config) : win
	});
}
