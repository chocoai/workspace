		    ...
		    <c:forEach items="${versionList}" var="version" varStatus="status">
			    <tr>
					...
					<td>
						...
						<c:if test="${version.softType=='2' }">在线安装包</c:if>
					</td>
					<td>
					...
					<td>
						...
						<a href="javascript:void(0);" onclick="detail('${version.id}')">编辑</a>
						<a href="javascript:void(0);" onclick="edit('${version.id}')">查看</a>
					
						
						<c:if test="${version.softType=='0' || version.softType=='1'}">
							<c:if test="${version.fileType=='1'}">
							
							<a href="javascript:void(0);" class="mgr10" onclick="unzip('${version.id}')">解压</a>
								
							</c:if>
							<c:if test="${version.fileType=='0' }">
								<c:if test="${version.isleveup=='0' && version.status=='0'}">
									<a href="javascript:void(0);" class="mgr10" onclick="openDownload('${version.id}','${version.softType}')">开放下载</a>
						...
						<c:if test="${version.softType=='2'}">
							<a href="javascript:void(0);" class="mgr10" onclick="softComponentList('${version.id}')">开放下载</a>
						</c:if>
					</td>
			    </tr>
		    </c:forEach>
