...
<div class="mg15 txq_main">
	<form id="pageForm"  name="pageForm" action="${ctx}/manage/browserLabel/list" method="post">
	<input type="hidden" id="search_type" name="search_type"/>
    <div class="mgtb10 txq_txtbox clearfix">
        
    </div>
    <div class="mgtb10">
		<div class="base_title"><strong>标签列表(版本号：${version })</strong></div>
	...
	<div class="base_page clearfix">
		...
		<c:if test="${not empty page.totalPage && page.totalPage > 1 }">
				...
				action="${ctx}/manage/browserLabel/list"></newTag:page>
		</c:if>
	</div>
	</form>
</div>
...
<div class="popup jumpBox add dis_none txq_main" style="width:700px;margin-top:200px">
    ...
    <div class="cont pd10">
		<table class="small_space data_list" width="100%" style="border-collapse:collapse;">
           <tr>
               <td width="15%" align="right"><span style="color: red;">*</span>网址名称：</td>
               <td width="35%" align="left"  colspan="3">
				<input type="text" name="name" class="inp focus"/>
				<span class="red"></span>
               </td>
           ...
           <tr>
               <td width="15%" align="right"><span style="color: red;">*</span>网址logo：</td>
               <td width="35%" align="left"  colspan="3">
				<input type="text" name="logo" class="inp focus"/>
				<span class="red"></span>
               </td>
               
           </tr>
           <tr>
               <td width="15%" align="right"><span style="color: red;">*</span>分组ID：</td>
               <td width="35%" align="left"  colspan="3">
				<input type="text" name="groupId" class="inp focus"/>
				<span class="red"></span>
               </td>
               
           </tr>
           <tr>
               <td width="15%" align="right"><span style="color: red;">*</span>分组名称：</td>
               <td width="35%" align="left"  colspan="3">
				<input type="text" name="groupName" class="inp focus"/>
				<span class="red"></span>
               </td>
    ...
    <div class="cont pd10">
		<table class="small_space data_list" width="100%" style="border-collapse:collapse;">
           ...
           <tr>
               ...
               <td align="left" colspan="3">
               	<textarea name="memo" rows="5" cols="48" placeholder="请输入软件更新描述......"></textarea>
               	<span class="red"></span>
               </td>
           </tr>
...
$(document).ready(function(){ 
	...
	$(".add input[name='save']").click(function(){
		...
		var logo=$(".add input[name='logo']").val();
		
		var groupId = $(".add input[name='groupId']").val();
		var groupName = $(".add input[name='groupName']").val();
		...
		if(name==''){
			$(".add input[name='name']").siblings(".red").text("请填写网址名称");
			return ;
		}
		...
		if(logo==''){
			$(".add input[name='logo']").siblings(".red").text("请填写网址logo");
			return ;
		}
		
		if(groupId==''){
			$(".add input[name='groupId']").siblings(".red").text("请填写分组ID");
			return ;
		}
	
		if(groupName==''){
			$(".add input[name='groupName']").siblings(".red").text("请填写分组名称");
			return;
		}
		...
		setTimeout(function(){
			$.ajax({
				type : "POST",
				url : "${ctx}/manage/browserLabel/save",
				data : {
					"name"  : name,
					"address"  : address,					
					"logo"  : logo,
...
function del(flatModelIds,id){
	...
	$.confirm("确定要删除该标签吗？",function(){
		$.ajax({
			type : "POST",
			url : "${ctx}/manage/browserLabel/delete",
			data : {
				"id"  : id,
