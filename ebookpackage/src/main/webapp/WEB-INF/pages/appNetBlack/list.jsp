...
<div class="mg15 txq_main">
    ...
    <div class="mgtb10">
		<div class="base_title"><strong>应用网业黑名单列表</strong></div>
...
function updateStatus(id,status){
	$.ajax({
		type : "POST",
		url : "${ctx}/manage/appNetBlackList/updateStatus",
		data : {
			"id"  : id,
			"status"  : status
		},
...
function edit(id){
	...
	$.ajax({
		type : "POST",
		url : "${ctx}/manage/appNetBlackList/appBlackListPlatformInfo",
		data : {"id" : id},
		async : false,
		dataType : 'json',
		success : function(data) {
	...
	$.ajax({
		type : "POST",
		url : "${ctx}/manage/appNetBlackList/detail",
		data : {"id" : id},
		async : false,
		dataType : 'json',
...
$(document).ready(function(){ 
	...
	$(".add input[name='save']").click(function(){
		...
		setTimeout(function(){
			$.ajax({
				type : "POST",
				url : "${ctx}/manage/appNetBlackList/save",
				data : {
					"name"  : name,
					"pkg"  : pkg,					
					"memo"  : memo,
	...
	$(".edit input[name='save']").click(function(){
		...
		setTimeout(function(){
			$.ajax({
				type : "POST",
				url : "${ctx}/manage/appNetBlackList/update",
				data : {
					"id"  : id,
					"name"  : name,
					"memo"  : memo,
...
function del(id){
	$.confirm("确定要删除该黑名单吗？",function(){
		$.ajax({
			type : "POST",
			url : "${ctx}/manage/appNetBlackList/delete",
			data : {
				"id"  : id
			},
			async : false,
