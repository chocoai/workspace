...
<div class="mg15 txq_main">
	<form id="pageForm"  name="pageForm" action="${ctx}/manage/netWorkBlackList/list" method="post">
	<input type="hidden" id="search_type" name="search_type"/>
    <div class="mgtb10 txq_txtbox clearfix">
        
    </div>
    <div class="mgtb10">
		<div class="base_title"><strong>黑名单列表</strong></div>
	...
	<div class="base_page clearfix">
		...
		<c:if test="${not empty page.totalPage && page.totalPage > 1 }">
				...
				action="${ctx}/manage/netWorkBlackList/list"></newTag:page>
		</c:if>
	</div>
	</form>
</div>
...
<div class="popup jumpBox add dis_none txq_main" style="width:700px;margin-top:200px;">
    ...
    <div class="cont pd10">
		<table class="small_space data_list" width="100%" style="border-collapse:collapse;">
           <tr>
               <td width="15%" align="right"><span style="color: red;">*</span>网址：</td>
               <td width="35%" align="left"  colspan="3">
				<input type="text" name="netWorkAddress" class="inp focus"/>
               </td>
...
<div class="popup jumpBox edit dis_none txq_main" style="width:700px;">
    ...
    <div class="cont pd10">
		<table class="small_space data_list" width="100%" style="border-collapse:collapse;">
           <tr>
               <td width="15%" align="right"><span style="color: red;">*</span>地址：</td>
               <td width="35%" align="left" colspan="3">
				<input type="text" name="netWorkAddress" class="inp focus"/>
               </td>
...
function updateStatus(id,status){
	$.ajax({
		type : "POST",
		url : "${ctx}/manage/netWorkBlackList/updateStatus",
		data : {
			"id"  : id,
			"status"  : status
		},
...
function edit(id){
	...
	$.ajax({
		type : "POST",
		url : "${ctx}/manage/netWorkBlackList/netWorkBlackListPlatformInfo",
		data : {"id" : id},
		async : false,
		dataType : 'json',
		success : function(data) {
	...
	$.ajax({
		type : "POST",
		url : "${ctx}/manage/netWorkBlackList/detail",
		data : {"id" : id},
		async : false,
		dataType : 'json',
		success : function(app) {
			if(app != null){
				...
				$(".edit input[name='netWorkAddress']").val(app.netWorkAddress);
...
$(document).ready(function(){ 
	...
	$(".add input[name='save']").click(function(){
		var netWorkAddress=$(".add input[name='netWorkAddress']").val();
		var memo=$(".add textarea[name='memo']").val();
		
		
		
		
		if($.trim(netWorkAddress)==''){
			$(".add input[name='netWorkAddress']").siblings(".red").text("请填写网络地址");
			return;
		}
		...
		setTimeout(function(){
			$.ajax({
				type : "POST",
				url : "${ctx}/manage/netWorkBlackList/save",
				data : {
	...
	$(".edit input[name='save']").click(function(){
		...
		var netWorkAddress=$(".edit input[name='netWorkAddress']").val();
		var memo=$(".edit textarea[name='memo']").val();
		
		
		//if($.trim(memo)==''){
		//	$(".edit textarea[name='memo']").siblings(".red").text("请填写描述");
		//	return;
		//}
		...
		setTimeout(function(){
			$.ajax({
				type : "POST",
				url : "${ctx}/manage/netWorkBlackList/update",
				data : {
					"id"  : id,
					"name"  : name,
					"memo"  : memo,
...
function del(id){
	$.confirm("确定要删除该黑名单吗？",function(){
		$.ajax({
			type : "POST",
			url : "${ctx}/manage/netWorkBlackList/del",
			data : {
				"id"  : id
			},
			async : false,
