...
<div class="mg15 txq_main">
	<form id="pageForm"  name="pageForm" action="${ctx}/manage/appBlackList/list" method="post">
	<input type="hidden" id="search_type" name="search_type"/>
    <div class="mgtb10 txq_txtbox clearfix">
        
    </div>
    <div class="mgtb10">
		<div class="base_title"><strong>应用网络黑名单列表</strong></div>
	...
	<div class="base_page clearfix">
		...
		<c:if test="${not empty page.totalPage && page.totalPage > 1 }">
				...
				action="${ctx}/manage/appBlackList/list"></newTag:page>
		</c:if>
	</div>
	</form>
</div>
    ...
    <div class="cont pd10">
		<table class="small_space data_list" width="100%" style="border-collapse:collapse;">
           ...
           <tr class="version_code_2">
               <td align="right" width="25%"><span style="color: red;">*</span>软件包：</td>
               <td align="left" width="25%" colspan="3">
               		<input type="text" name="pkg" class="inp focus"/>
               		<span class="red"></span>
               </td>
           </tr>
...
var uploader = new plupload.Uploader({ //实例化一个plupload上传对象
	...
	init: {
		...
		FileUploaded:function(uploader,file,responseObject){
			...
			$(".add span[name='version_code']").text(app.versionCode);z
		},
...
function updateStatus(id,status){
	$.ajax({
		type : "POST",
		url : "${ctx}/manage/appBlackList/updateStatus",
		data : {
			"id"  : id,
			"status"  : status
		},
...
function edit(id){
	...
	$.ajax({
		type : "POST",
		url : "${ctx}/manage/appBlackList/appBlackListPlatformInfo",
		data : {"id" : id},
		async : false,
		dataType : 'json',
		success : function(data) {
	...
	$.ajax({
		type : "POST",
		url : "${ctx}/manage/appBlackList/detail",
		data : {"id" : id},
		async : false,
		dataType : 'json',
...
$(document).ready(function(){ 
	...
	$(".add input[name='save']").click(function(){
		...
		if(name==''){
			$(".add input[name='pkg']").siblings(".red").text("请填写软件名称");
			return;
		}
		...
		setTimeout(function(){
			$.ajax({
				type : "POST",
				url : "${ctx}/manage/appBlackList/save",
				data : {
					"name"  : name,
					"pkg"  : pkg,					
					"memo"  : memo,
					"interVersionCode" : inter_version_code,
	...
	$(".edit input[name='save']").click(function(){
		...
		setTimeout(function(){
			$.ajax({
				type : "POST",
				url : "${ctx}/manage/appBlackList/update",
				data : {
					"id"  : id,
					"name"  : name,
					"memo"  : memo,
...
function del(id){
	$.confirm("确定要删除该黑名单吗？",function(){
		$.ajax({
			type : "POST",
			url : "${ctx}/manage/appBlackList/delete",
			data : {
				"id"  : id
			},
			async : false,
