...
<div class="mg15 txq_main">
		    ...
		    <c:forEach items="${list}" var="obj" varStatus="status">
			    <tr>
					...
					<td title="${obj.role_name }">
						...
						<c:if test="${not empty obj.role_name && fn:length(obj.role_name)<=25}">${fn:substring(obj.role_name,1,fn:length(obj.role_name))}</c:if>
	...
	<div class="base_page clearfix">
		...
		<c:if test="${not empty page.totalPage && page.totalPage > 1 }">
				...
				action="${ctx}/sys/user/list"></newTag:page>
		</c:if>
	</div>
	</form>
</div>
...
<div class="popup jumpBox importexcel dis_none">
    ...
    <div class="cont pd10">
    <form action="${ctx}/sys/user/importexcel"  method="post" enctype ="multipart/form-data" id="importexcel" > 
    <table class="small_space data_list" width="100%" style="border-collapse:collapse;">
    <tr>
     <td align="right"><span style="color: red;"></span>下载导入模板：</td>
     <td align="left">
     <a href="${ctx}/sys/user/uploadexcel">管理员信息模板</a>
     </td>
     </tr>
     <tr>
...
function enableAndDisable(status,id){
	...
	$.ajax({
		type : "POST",
		url : "${ctx}/sys/user/enableAndDisable",
...
function exportexcel(){
	...
	window.location.href='${ctx}/sys/user/exportexcel?name='+name+'&type='+type+'&status='+status;
}
...
function grantRole(user_id){
	...
	$.ajax({
		type : "POST",
		url : "${ctx}/sys/role/queryUserRole",
