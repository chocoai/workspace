...
<div class="mg15 txq_main">
	<form id="pageForm"  name="pageForm" action="${ctx}/manage/whiteList/list" method="post">
	<input type="hidden" id="search_type" name="search_type"/>
    <div class="mgtb10 txq_txtbox clearfix">
        
    </div>
    <div class="mgtb10">
		<div class="base_title"><strong>白名单列表</strong></div>
	...
	<div class="base_page clearfix">
		...
		<c:if test="${not empty page.totalPage && page.totalPage > 1 }">
				...
				action="${ctx}/manage/whiteList/list"></newTag:page>
		</c:if>
	</div>
	</form>
</div>
...
$(document).ready(function(){ 
	...
	$(".add input[name='save']").click(function(){
		...
		if(name==''){
			$(".add input[name='name']").siblings(".red").text("请填写软件名称");
			return ;
		}
...
function del(id){
	$.confirm("确定要删除该白名单吗？",function(){
		$.ajax({
			type : "POST",
			url : "${ctx}/manage/whiteList/delete",
			data : {
				"id"  : id
			},
			async : false,
