...
<div class="mg15 txq_main">
	<form id="pageForm"  name="pageForm" action="${ctx}/sys/role/list" method="post">
	...
	<div class="base_page clearfix">
		...
		<c:if test="${not empty page.totalPage && page.totalPage > 1 }">
				...
				action="${ctx}/sys/role/list"></newTag:page>
		</c:if>
	</div>
	</form>
</div>
...
function editPermision(role_id) {
	...
	$(function(){
		$(".editPermision input[name='save']").click(function(){
            ...
            $.ajax({
				type : "POST",
				url : "${ctx}/sys/role/saveRolePermission",
				data : {
					"role_id" : role_id,
...
function enableAndDisable(status,id){
	...
	$.ajax({
		type : "POST",
		url : "${ctx}/sys/role/enableAndDisable",
