...
<div class="mg15 txq_main">
	<form id="pageForm"  name="pageForm" action="${ctx}/manage/licenceOrg/softLicenceList" method="post">
	<!-- 查询条件 -->
	...
	<div class="base_page clearfix">
		...
		<c:if test="${not empty page.totalPage && page.totalPage >= 1 }">
				...
				action="${ctx}/manage/licenceOrg/softLicenceList?ORG_ID=${orgId}&SCHOOL_NAME=${schoolName}"></newTag:page>
		</c:if>
	</div>
	</form>
</div>
...
function exportSearchExcel(){
	...
	window.location.href='${ctx}/manage/licenceOrg/exportSearchExcel2?ORG_ID='+orgId+'&SCHOOL_NAME='+schoolName+'&licence_code='+licence_code+'&status='+status;
...
function edit(id){
	...
	$.ajax({
		type : "POST",
		url : "${ctx}/manage/licenceOrg/detailEbpSoftLicence",
		data : {"id" : id},
		async : false,
		dataType : 'json',
...
//	}
	...
	setTimeout(function(){
		$.ajax({
			type : "POST",
			url : "${ctx}/manage/licenceOrg/saveEbpSoftLicence",
			data : {
