...
@Controller
@RequestMapping("/manage/countWidget")
public class CountWidgetController extends BaseController {
	...
	public String list(HttpServletRequest request, Model model) throws Exception {
		...
		if (paramMap.get("cityCode") != null && StringUtils.isNotEmpty(paramMap.get("cityCode").toString())) {
			...
			List<Map> list = countWidgetService.getOrgListByAreaCode(paramMap);
			model.addAttribute("orgList", JSONArray.fromObject(list));
		}

		countWidgetService.addCountDataToAttribute(model);

		countWidgetService.addAreaCountDataToAttribute(model, paramMap);

		if (paramMap.get("provinceCode") == null) {
			Map<String, Object> param = new HashMap<String, Object>();
			param.put("year", year);
			param.put("month", month);
			currentMonthWidgetHistoryList = countWidgetService.getWidgetHistoryCount(param);// 找到所有区域的统计

			// 上月数据
			param.put("year", preYear);
			param.put("month", preMonth);
			lastMonthWidgetHistoryList = countWidgetService.getWidgetHistoryCount(param);
			...
			currentMonthWidgetHistoryList = countWidgetService.getWidgetHistoryCount(param);// 学校下所有用户的统计
			...
			param.put("areaCode", paramMap.get("cityCode"));
			param.put("orgId", paramMap.get("orgId"));
			lastMonthWidgetHistoryList = countWidgetService.getWidgetHistoryCount(param);// 区域下所有学校的统计
			...
			param.put("month", month);
			param.put("areaCode", paramMap.get("cityCode"));
			currentMonthWidgetHistoryList = countWidgetService.getWidgetHistoryCount(param);// 区域下所有学校的统计
	...
	public void exportExcel(HttpServletRequest request, HttpServletResponse response, Model model) throws Exception {
		...
		String[] tableHeader = { "控件名称|widgetName", "使用次数|useCount", "上月使用次数|previousUseCount", "增长率|rate",
				"描述|description" };
	...
	public void queryOrg(HttpServletRequest request, HttpServletResponse response, Model model) throws Exception {
		...
		List<Map> list = countWidgetService.getOrgListByAreaCode(param);
	...
	public void chatesData(HttpServletRequest request, HttpServletResponse response, Model model) throws Exception {
		...
		if (provinceCode == null || "".equals(provinceCode)) {// 所有区域的排名
			widgetHistoryList = countWidgetService.getAllRanking(paramMap);
		} else if (orgId != null && StringUtils.isNotEmpty(orgId)) {// 该学校所有老师的排名
			paramMap.put("orgId", orgId);
			widgetHistoryList = countWidgetService.getUserRankingByOrg(paramMap);
		} else if (cityCode != null && StringUtils.isNotEmpty(cityCode)) {// 该区域下所有学校的排名
			paramMap.put("cityCode", cityCode);
			widgetHistoryList = countWidgetService.getOrgRankingByAreaCode(paramMap);
		} else if (provinceCode != null && StringUtils.isNotEmpty(provinceCode)) {// 本省份下所有区域的排名
			paramMap.put("parentId", provinceCode);
			widgetHistoryList = countWidgetService.getAreaCodeRankingByProvinceCode(paramMap);
		}
