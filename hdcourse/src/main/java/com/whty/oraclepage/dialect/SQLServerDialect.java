package com.whty.oraclepage.dialect;
...
public class SQLServerDialect extends Dialect {
	...
	private String getSQLByTempTable(String querySqlString, int offset, int limit) {
		...
		// boolean
		// bDistinct=sSubTable.substring(6).trim().toLowerCase().startsWith("distinct");
		// sSubTable =
		// sSubTable.substring(bDistinct?(sSubTable.indexOf("distinct")+8):6).trim();
		//
		// sSubTable="select "+(bDistinct?"distinct":"")+" top 100 percent
		// "+sSubTable;
		//
		// sql="set nocount on\n";
		// sql+="select temp_tbl.*,IDENTITY(int,1,1) as temp_ident_field into
		// "+sTempTableName+" \nFrom (\n"+sSubTable+") as temp_tbl \n";
		// if(sTempOrder.length()>0){
		// sql+=" order by "+sTempOrder+" \n";
		// }
		// sql+="select * from "+sTempTableName+" where temp_ident_field between
		// "+((this.nCurrentPage-1)*this.nPageSize+1)+" and
