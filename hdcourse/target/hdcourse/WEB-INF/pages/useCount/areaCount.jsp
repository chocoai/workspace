...
<body>
<div class="bo_main">
	<form id="pageForm"  name="pageForm" action="${ctx}/manage/countLogin/areaCount" method="post">
   ...
   <div class="code_table data_index">
        <table class="mod_table t_c" width="100%" style="border-collapse:collapse;">
            <tr>
                <td width="10%">
                	<p>区域</p>
                    <p class="num"><span class="cf00">${areaNum}个</span></p>
                </td>
                <td width="10%">
                	<p>总次数</p>
                    <p class="num"><span class="c00f">${areaUseTotal}次</span></p>
                </td>
                <td  width="10%">
                	<p>学校</p>
                	<p class="num"><span class="c000">${orgTotal}个</span></p>
                </td>
            </tr>
        </table>
    </div>
           ...
           <tr>
             ...
             <c:forEach items="${areaCountList}" var="areaCount" varStatus="status">
             	...
             	<tr>
             		...
             		<td>${areaCount.useCount}</td>
             		<td>${areaCount.previousUseCount}</td>   
             		<td>${time }</td>
             		<td>${areaCount.rate}</td>
             		<td>${areaCount.orgUseRanking}</td>
    ...
    <div class="base_page clearfix">
		<c:if test="${fn:length(areaCountList) > 0}">
				...
				action="${ctx}/manage/countLogin/areaCount"></newTag:page>
		</c:if>
	</div>
	</form>
</div>
...
function exportAreaCountExcel(){
	...
	window.location.href='${ctx}/manage/countLogin/exportAreaCountExcel?province=' + province+'&start_time='+start_time;
}
	...
	var optionA = { 
		 legend: {
			...
			data:(function(){
                ...
                type : "post",
                async : false, //同步执行
                url : "${ctx}/manage/countLogin/getAreaCounteLgendData",
        ...
        series: [
                 {
                     ...
                     data:(function(){
                        ...
                        type : "post",
                        async : false, //同步执行
                        url : "${ctx}/manage/countLogin/getAreaCountChartsData",
                        data : {province:$("#province").val(),start_time:$(".search input[name='start_time']").val()},
                        dataType : "json", //返回数据形式为json
                        success : function(result) {
                        if (result) {
