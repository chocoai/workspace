...
<body>
<div class="bo_main">
	<form id="pageForm"  name="pageForm" action="${ctx}/manage/countLogin/userCount" method="post">
   ...
   <div class="code_table data_index">
        <table class="mod_table t_c" width="100%" style="border-collapse:collapse;">
            <tr>
                <td width="10%">
                	<p>使用人数</p>
                    <p class="num"><span class="cf00">${userNum}个</span></p>
                </td>
                <td width="10%">
                	<p>使用次数</p>
                    <p class="num"><span class="c00f">${userTotal}次</span></p>
                </td>
            </tr>
        </table>
    </div>
    ...
    <div class="code_stateBox">
  	...
  	<div class="bo_title mgt25">
        <h3><i class="ico ico3"></i>当月数据报表</h3>
        <div class="sort fr">
        ...
        <table class="small_space data_list" width="100%" style="border-collapse:collapse;">
            ...
            <c:forEach items="${userCountList}" var="userCount" varStatus="status">
             	<tr>
             		...
             		<td>${time}</td>
             		<td>${userCount.useCount}</td>
             		<td>${userCount.previousUseCount}</td>
    ...
    <div class="base_page clearfix">
		...
		<c:if test="${fn:length(userCountList) > 0}">
				...
				action="${ctx}/manage/countLogin/userCount"></newTag:page>
		</c:if>
	</div>
	</form>
</div>
		...
		<div class="cont pd10">
			<table class="small_space data_list" width="100%"
				...
				<tr>
					...
					<td width="15%" align="right">所属平台：</td>
...
function userInfo(orgId,userId){
	...
	$.ajax({
		type : "POST",
		url : "${ctx}/manage/countLogin/userInfo",
...
function exportUserCountExcel(){
	...
	window.location.href='${ctx}/manage/countLogin/exportUserCountExcel?province=' + province+'&cityCode='+city+'&orgId='+org+'&start_time='+start_time+'&orgName='+orgName;
}
...
$("#city2").bind("change", function(){
	if($("#city2").val()==''){
		$('#org').html('<option value="">全部</option>');
		return;		
	}
	$('#org').html(queryOrg($("#city2").val()));
}); 
	...
	var optionA = { 
        ...
        yAxis: {
            ...
            data: (function(){
		        ...
		        type : "post",
		        async : false, //同步执行
		        url : "${ctx}/manage/countLogin/getUseCountChartsData",
		        data : {province:$("#province").val(),city2:$("#city2").val(),org:$("#org").val(),start_time:$(".search input[name='start_time']").val()},
