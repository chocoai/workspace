...
<h2 class="adminTit bor_bottom"><em>授权单位列表</em><span class="fr"></span></h2>
	...
	<form id="pageForm"  name="pageForm" action="${ctx}/manage/licenceOrg/list" method="post">
	...
	<div class="base_page clearfix">
		...
		<c:if test="${not empty page.totalPage && page.totalPage >= 1 }">
				...
				action="${ctx}/manage/licenceOrg/list"></newTag:page>
		</c:if>
	</div>
	</form>
</div>
...
function detail(id){
	$.ajax({
		type : "POST",
		url : "${ctx}/manage/licenceOrg/detail",
		data : {"id" : id},
		async : false,
		dataType : 'json',
...
$("#updateSubmit").click(function(){
	...
	setTimeout(function(){
		$.ajax({
			type : 'POST',
			url : '${ctx}/manage/licenceOrg/update',
			data : {
				"id":id,
			...
			error:function(){
				alert("修改失败！");
				window.location.href='${ctx}/manage/licenceOrg/list';
			}
		});
	},1);
});
...
$("#addSubmit").click(function(){
	...
	setTimeout(function(){
		$.ajax({
			type : 'POST',
			url : '${ctx}/manage/licenceOrg/save',
...
{
	window.location.href='${ctx}/manage/licenceOrg/softLicenceList?ORG_ID='+orgId+'&SCHOOL_NAME='+orgName;
}
...
function delList(){
	...
	$.confirm("确定要删除选中机构授权信息吗？",function(){
		$.ajax({
			type : "POST",
			url : "${ctx}/manage/licenceOrg/delete",
			data : {"ids" : ids},
			async : false,
			dataType : 'text',
			success : function(msg) {
