...
<div class="mg15 txq_main">
	<form id="pageForm"  name="pageForm" action="${ctx}/manage/softLicence/softLicenceList" method="post">
	<!-- 查询条件 -->
...
<%--     	<div class="base_title">
			<strong><a class="mgr10" href="${ctx}/manage/licenceOrg/list" name="wddx" style="color: #497cc0;">返回上一级</a><span id ="SCHOOL_NAME" style="font-weight:bold;">${schoolName}</span>&nbsp;账号授权管理</strong>
		</div> --%>
		    ...
		    <c:forEach items="${softLicenceList}" var="g" varStatus="status">
			    <tr>
					...
					<td>${g.max_use_count }</td>
 					<td><fmt:formatDate value="${g.start_time }" pattern="yyyy-MM-dd" />
	          			<c:choose>
		            		<c:when test="${empty g.end_time }">无</c:when>
		            		<c:otherwise>至<fmt:formatDate value="${g.end_time }" pattern="yyyy-MM-dd" /></c:otherwise>
	            		</c:choose>
					</td> 
					<td>
	...
	<div class="base_page clearfix">
		...
		<c:if test="${not empty page.totalPage && page.totalPage >= 1 }">
				...
				action="${ctx}/manage/softLicence/softLicenceList?ORG_ID=${orgId}&SCHOOL_NAME=${schoolName}"></newTag:page>
		</c:if>
	</div>
	</form>
</div>
...
<!--详情-->
<div class="popup jumpBox detail dis_none" style="width: 900px;height: 520px;">
	<div class="tit"><span class="fl">编辑激活码</span><span class="close" name="close">X</span></div>
	<div class="cont pd10" style="width: 880px;height: 450px;">
		<div style="width:50%; height:450px;overflow :auto;float: left">
		<table class="small_space data_list" width="100%" style="border-collapse:collapse;">
           <tr>
           ...
           <tr>
				<td align="right">激活为正式授权:</td>
				   <td align="left">
					<input type="text" name="start_time" readonly="readonly" class="inp focus" style="width: 100px;"/>
					<input type="text" id="d4321" class="dis_none"/>
					<img onclick="WdatePicker({el:'d4321',onpicked:startTimePickedFunc2,oncleared:clearStartTime2,maxDate:'#F{$dp.$D(\'d4322\')||\'2020-12\'}'})" src="${ctx}/js/My97DatePicker/skin/datePicker.gif" width="20" height="26">
					至
					 <input type="text" name="end_time" readonly="readonly" class="inp focus" style="width: 100px;"/>
					<input type="text" id="d4322" class="dis_none"/>
					<img onclick="WdatePicker({el:'d4322',onpicked:endTimePickedFunc2,oncleared:clearEndTime2,minDate:'#F{$dp.$D(\'d4321\')}'})" src="${ctx}/js/My97DatePicker/skin/datePicker.gif" width="20" height="26">
					<span class="red"></span>
				</td>
           </tr>
           <tr>
		...
		<div style="width:40%; height:400px;overflow :auto;float: left">
...
<!--新增-->
<div class="popup jumpBox add_edit dis_none" style="width: 900px;height: 370px;">
    <div class="tit"><span class="fl">新增激活码</span><span class="close" name="close">X</span></div>
    <div class="cont pd10" style="width: 880px;height: 300px;">
		...
		<table class="small_space data_list" width="100%" style="border-collapse:collapse;">
           ...
           <tr>
				<td align="right"><span style="color: red;">*</span>授权有效期：</td>
           		<td align="left">
           		<select id="op" name="limit">
           			<option value="0">请选择使用期限</option>
...
<%--
		...
		<div style="width:40%; height:300px;overflow :auto;float: left">
...
function exportSearchExcel(){
	...
	window.location.href='${ctx}/manage/softLicence/exportSearchExcel?ORG_ID='+orgId+'&SCHOOL_NAME='+schoolName+'&licence_code='+licence_code+'&status='+status+'&limit_day='+limit_day;
...
function startTimePickedFunc2(){
	$(".detail input[name='start_time']").val($('#d4321').val());
}

function endTimePickedFunc2(){
	var endTime = $('#d4322').val();
	var year = endTime.split("-")[0];
	var month = endTime.split("-")[1];
	$(".detail input[name='end_time']").val(endTime);
}

function clearStartTime2(){
	$(".detail input[name='start_time']").val('');
}

function clearEndTime2(){
	$(".detail input[name='end_time']").val('');
}
	...
	$.ajax({
		...
		success : function(soft) {
			if(soft != null && soft !=''){
				...
				$(".detail input[name='start_time']").val(toDate(soft.start_time,"yyyy-MM-dd"));
				$(".detail input[name='end_time']").val(soft.end_time ? toDate(soft.end_time,"yyyy-MM-dd"):'永久');
...
$(".add_edit input[name='save']").click(function(){
	...
	var limit_day = $("#op option:selected").val();
	var orgId='e27285760d264b9b9279ad3bb3a56755';
	var ebpAuth;
	var sdpAuth;
	var amAuth;
...
/* 				"start_time"  : startTime,
			...
			success : function(msg) {
				...
				var url="exportSearchExcel?ids="+msg;
...
$(".detail input[name='update']").click(function(){
	...
	var startTime=$(".detail input[name='start_time']").val();
	var endTime=$(".detail input[name='end_time']").val();
...
function getTree1() {
    ...
    $.ajax({
        type : "POST",
        url : "${ctx}/manage/licenceOrg/getCodeTree",
        async : true,
        dataType : 'json',
        success : function(msg) {
