...
<body>
<div class="bo_main">
<form id="pageForm"  name="pageForm" action="${ctx}/manage/countUseTaking/orgCount" method="post">
   ...
   <div class="code_table data_index">
        <table class="mod_table t_c" width="100%" style="border-collapse:collapse;">
            <tr>
                ...
                <td width="10%">
                	<p>总时长</p>
                    <p class="num"><span class="c00f">${orgTakingTotal} </span></p>
                </td>
                <td  width="10%">
                	<p>老师数量</p>
                	<p class="num"><span class="c000">${userTotalCount}个</span></p>
                </td>
            </tr>
        </table>
    </div>
        ...
        <table class="small_space data_list" width="100%" style="border-collapse:collapse;">
           ...
           <c:forEach items="${orgCountList}" var="orgCount" varStatus="status">
             	<tr>
             		...
             		<td>${time}</td>
             		<td>${orgCount.useTakingStr}</td>
             		<td>${orgCount.previousUseTakingStr}</td>
             		<td>${orgCount.rate}</td>
             		<td>${orgCount.userNum}</td>
    ...
    <div class="base_page clearfix">
		...
		<c:if test="${fn:length(orgCountList) > 0}">
				...
				action="${ctx}/manage/countUseTaking/orgCount"></newTag:page>
		</c:if>
	</div>
	</form>
</div>
...
function exportOrgCountExcel(){
	...
	window.location.href='${ctx}/manage/countUseTaking/exportOrgTakingCountExcel?province=' + province+'&cityCode='+city+'&start_time='+start_time+"&orgName="+orgName;
}
	...
	var optionA = { 
        ...
        yAxis: {
            ...
            data: (function(){
		        ...
		        type : "post",
		        async : false, //同步执行
		        url : "${ctx}/manage/countUseTaking/getOrgUseTakingChartsData",
