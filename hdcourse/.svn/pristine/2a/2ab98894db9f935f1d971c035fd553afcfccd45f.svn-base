...
<body>
<div class="bo_main">
	<form id="pageForm"  name="pageForm" action="${ctx}/manage/userActivityCount/areaCount" method="post">
   ...
   <div class="code_table data_index">
        <table class="mod_table t_c" width="100%" style="border-collapse:collapse;">
            <tr>
                ...
                <td width="10%">
                	<p>总时长</p>
                    <p class="num"><span class="c00f">${loginToalCount}</span></p>
                </td>
                <td  width="10%">
                	<p>学校</p>
            ...
            <tr>
             ...
             <c:forEach items="${areaCountList}" var="areaCount" varStatus="status">
             	<tr>
             		...
             		<td>${areaCount.loginTakingStr}</td>
             		<td>${areaCount.previousLoginTakingStr}</td>
             		<td>${time}</td>
             		<td>${areaCount.rate}</td>
             		<td>${areaCount.orgLoginTakingRanking}</td>
    ...
    <div class="base_page clearfix">
		<c:if test="${fn:length(areaCountList) > 0}">
				...
				action="${ctx}/manage/userActivityCount/areaCount"></newTag:page>
		</c:if>
	</div>
	</form>
</div>
...
function look(areaCode){
	...
	window.location.href="orgCount?cityCode="+areaCode;
}
...
function exportAreaCountExcel(){
	...
	window.location.href='${ctx}/manage/userActivityCount/exportAreaActivityCountExcel?province=' + province+'&start_time='+start_time;
}
	...
	var optionA = { 
		 legend: {
			...
			data:(function(){
                ...
                type : "post",
                async : false, //同步执行
                url : "${ctx}/manage/userActivityCount/getActivityCountLegendData",
        ...
        series: [
                 {
                     ...
                     data:(function(){
                        ...
                        type : "post",
                        async : false, //同步执行
                        url : "${ctx}/manage/userActivityCount/getAreaActivityChartsData",
                        data : {province:$("#province").val(),start_time:$(".search input[name='start_time']").val()},
                        dataType : "json", //返回数据形式为json
                        success : function(result) {
                        if (result) {
