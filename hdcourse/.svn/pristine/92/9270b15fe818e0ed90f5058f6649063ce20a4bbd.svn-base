...
public class CommonFunction {
	 ...
	 * @return
	 */
	public static String calculateStartAndEndTime(Date startTime, Date endTime, String flag) {
		if (startTime == null || endTime == null || isEmpty(flag))
			return null;

		return DurationFormatUtils.formatPeriod(startTime.getTime(), endTime.getTime(), flag);
	}
	...
	/**
	 * 登录接口用户信息sessionUser转后台用户实体Ta_user
	 * 
	 * @param sessionUser
	 * @return
	 */
	public static Ta_user apiUserToTA_user(ApiUser apiUser) {
		Ta_user ta_user = null;
		if (null != apiUser && null != apiUser.getUserId()) {
			ta_user = new Ta_user();
			ta_user.setAddress(apiUser.getAddress());
			ta_user.setArea_code(apiUser.getAreaId());
			ta_user.setArea_name(apiUser.getAreaName());
			ta_user.setBirthday(apiUser.getBirthday());
			ta_user.setCreate_time(new Date());
			ta_user.setEmail(apiUser.getEmail());
			ta_user.setGender(apiUser.getGender());
			ta_user.setId(GUIDGenerator.getUUID32());
			ta_user.setLast_time(new Date());
			ta_user.setLogin_count(0);
			ta_user.setOrg_id(apiUser.getOrgId());
			ta_user.setOrg_name(apiUser.getOrgName());
			ta_user.setPhone_number(apiUser.getMobilePhone());
			ta_user.setPlatform_id(apiUser.getPlatformCode());
			ta_user.setReal_name(apiUser.getRealName());
			ta_user.setStatus("0");
			ta_user.setUpdate_time(new Date());
			ta_user.setUse_time(new Date());
			ta_user.setUser_account(apiUser.getAccount());
			ta_user.setUser_id(apiUser.getUserId());
			ta_user.setUser_name(apiUser.getAccount());
			ta_user.setUser_type(apiUser.getUserType());
		}
		return ta_user;
	}
	...
	public static void main(String[] args) {
		...
		System.out.println(formatDuring(null));
	}
