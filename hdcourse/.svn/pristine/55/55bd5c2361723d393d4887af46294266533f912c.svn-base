...
<div class="mg15 txq_main">
	<form id="pageForm"  name="pageForm" action="${ctx}/manage/behavior/dzsbList" method="post">
    ...
    <div class="mgtb10 txq_txtbox clearfix">
        ...
        <p class="fl mgr10"><span>班级：</span>
        	<input name="class_name" value="${class_name }" type="text" style="width:150px;" class="inp focus"/>
        </p>
    ...
    <div class="mgtb10">
		<div class="base_title"><strong>电子书包统计列表</strong></div>
		...
		<table class="small_space data_list" width="100%" style="border-collapse:collapse;">
			<tr>
				...
				<th width="10%">班级</th>
				<th width="8%">用户类型</th>
				<th width="8%">接入平板数量</th>
				<th width="10%">开始使用时间</th>
				<th width="10%">结束使用时间</th>
				<th width="10%">使用时长</th>
		    </tr>
	...
	<div class="base_page clearfix">
		...
		<c:if test="${fn:length(list) > 0}">
				...
				action="${ctx}/manage/behavior/dzsbList"></newTag:page>
		</c:if>
	</div>
	</form>
</div>
