...
<div class="mg15 txq_main">
	...
	<form id="pageForm"  name="pageForm" action="${ctx}/manage/activityCount/orgList" method="post">
		<div class="clearfix">
        ...
        <div class="mgtb10">
			<div class="base_title"><strong>学校使用时长统计</strong></div>
			...
			<div class="clearfix mgtb10">
			    ...
			    <c:forEach items="${activityCountList}" var="activityCount" varStatus="status">
			    	<tr>
			    		...
			    		<td>${activityCount.userName}</td>
			    		<td>${activityCount.areaName}</td>
			    		<td>
			    		
			    			<c:if test="${activityCount.useActivity=='0' }">不活跃</c:if>
			    			<c:if test="${activityCount.useActivity=='1' }">活跃</c:if>
			    		</td>
			    		<td>
			    			<c:if test="${activityCount.previousUseActivity=='0' }">不活跃</c:if>
			    			<c:if test="${activityCount.previousUseActivity=='1' }">活跃</c:if>
			    		</td>
		...
		<c:if test="${fn:length(activityCountList) > 0}">
				...
				action="${ctx}/manage/activityCount/orgList"></newTag:page>
		</c:if>
		</div>
	</form>
</div>
