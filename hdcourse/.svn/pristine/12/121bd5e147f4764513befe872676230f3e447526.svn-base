...
public class PoiExcelUtils {
	...
	private void writeContent(HSSFWorkbook hssfWorkbook, HSSFSheet hssfSheet, String[] headers, List<?> dataList,
		...
		for (int j = 0, size = dataList.size(); j < size; j++) {
			...
			for (int i = 0, len = headers.length; i < len; i++) {
				...
				// Map<String, Object> aa = (Map<String, Object>)
				// dataList.get(j);
				// String val = "";
				// if (aa.get(headers[i].split("@")[1]) != null) {
				// val = aa.get(headers[i].split("@")[1]).toString();
				// }

				cellValue = ReflectUtils.getValueOfGetIncludeObjectFeild(dataList.get(j), headers[i].split("@")[1]);
				// cellValue = val;
				// if (cellValue instanceof Date) {
				// cellValue = CommonFunction.getDateSampleString((Date)
				// cellValue,
				// CommonFunction.DEFAULT_TIME_SAMPLE_FORMAT);
				// }
	...
	private void writeExportDate(HSSFWorkbook hssfWorkbook, HSSFRow row, int cellIndex) {
		...
		cellValueHandler(cell4ExportDateValue, TimeUtil.dateTime2String(TimeUtil.getCurrentTime()));
	...
	public static HSSFWorkbook createExcel2ExportByAreaUseCount(String sheetName, String title, String[] headers,
			List<?> detailDataList, String date) throws Exception {
		if (ArrayUtils.isEmpty(headers)) {
			LOGGER.warn("【参数headers为空】");
			throw new IllegalArgumentException("headers");
		}
		...
		int useTotal = 0;
		int orgTotal = 0;
		for (Object bean : detailDataList) {
			if (bean instanceof AreaMonthUsageCount) {
				AreaMonthUsageCount count = (AreaMonthUsageCount) bean;

				useTotal = useTotal + count.getUseCount();
				orgTotal = orgTotal + count.getOrgUseRanking();
			}
		}
		...
		HSSFSheet hssfSheet = poiExcelUtil.getHSSFSheet(hssfWorkbook, sheetName);
		// 3.写标题
		// poiExcelUtil.writeTitle(hssfWorkbook, hssfSheet, headers, title);

		HSSFCellStyle titleCellStyle = poiExcelUtil.createContentCellStyle(hssfWorkbook);
		...
		HSSFCell yearCell = titleRow.createCell(1);
		// 设置标题文本
		yearCell.setCellValue(date);
		// 设置单元格样式
		yearCell.setCellStyle(titleCellStyle);

		// 区域统计
		HSSFRow countRow = hssfSheet.createRow(1);
		HSSFCell areaSum = countRow.createCell(0);
		areaSum.setCellValue(new HSSFRichTextString("区域"));
		areaSum.setCellStyle(titleCellStyle);

		HSSFCell areaSumValue = countRow.createCell(1);
		areaSumValue.setCellValue(new HSSFRichTextString(Integer.toString(detailDataList.size()) + "个"));
		areaSumValue.setCellStyle(titleCellStyle);

		HSSFCell areaUseTotle = countRow.createCell(2);
		areaUseTotle.setCellValue(new HSSFRichTextString("总计"));
		areaUseTotle.setCellStyle(titleCellStyle);

		HSSFCell areaUseTotleValue = countRow.createCell(3);
		areaUseTotleValue.setCellValue(new HSSFRichTextString(Integer.toString(useTotal)) + "次");
		areaUseTotleValue.setCellStyle(titleCellStyle);

		HSSFCell areaOrgSum = countRow.createCell(4);
		areaOrgSum.setCellValue(new HSSFRichTextString("学校"));
		areaOrgSum.setCellStyle(titleCellStyle);

		HSSFCell areaOrgSumValue = countRow.createCell(5);
		areaOrgSumValue.setCellValue(new HSSFRichTextString(Integer.toString(orgTotal)) + "个");
		areaOrgSumValue.setCellStyle(titleCellStyle);

		poiExcelUtil.writeHeader(hssfWorkbook, hssfSheet, headers, 2);

		poiExcelUtil.writeContent(hssfWorkbook, hssfSheet, headers, detailDataList, 3);
	...
	public static HSSFWorkbook createExcel2ExportByOrgUseCount(String sheetName, String provinceName, String cityName,
			String[] headers, List<?> detailDataList, String date) throws Exception {
		...
		LOGGER.info("【生成Excel,用于导出】sheetName : " + sheetName + " , title : 学校使用次数统计 , headers : "
				+ Arrays.toString(headers));

		int useTotal = 0;

		for (Object bean : detailDataList) {
			if (bean instanceof OrgMonthUsageCount) {
				OrgMonthUsageCount count = (OrgMonthUsageCount) bean;
				useTotal = useTotal + count.getUseCount();
			}
		}
		...
		HSSFRow titleRow = hssfSheet.createRow(0);
		titleRow.setHeight((short) 500);
		HSSFCell provinceCell = titleRow.createCell(0);
		// 设置标题文本
		provinceCell.setCellValue(new HSSFRichTextString(provinceName));
		// 设置单元格样式
		provinceCell.setCellStyle(titleCellStyle);

		HSSFCell cityCell = titleRow.createCell(1);
		// 设置标题文本
		cityCell.setCellValue(new HSSFRichTextString(cityName));
		// 设置单元格样式
		cityCell.setCellStyle(titleCellStyle);

		HSSFCell yearCell = titleRow.createCell(2);
		// 设置标题文本
		yearCell.setCellValue(date);
		// 设置单元格样式
		yearCell.setCellStyle(titleCellStyle);
		...
		HSSFRow countRow = hssfSheet.createRow(1);
		HSSFCell areaSum = countRow.createCell(0);
		areaSum.setCellValue(new HSSFRichTextString("学校数量"));
		areaSum.setCellStyle(titleCellStyle);
	...
	public static HSSFWorkbook createExcel2ExportByUseCount(String sheetName, String provinceName, String cityName,
			String orgName, String[] headers, List<?> detailDataList, String date) throws Exception {
		...
		for (Object bean : detailDataList) {
			if (bean instanceof UserMonthUsageCount) {
				UserMonthUsageCount count = (UserMonthUsageCount) bean;
				useTotal = useTotal + count.getUseCount();
			}
		}
		...
		HSSFCell orgNameCell = titleRow.createCell(2);
		// 设置标题文本
		orgNameCell.setCellValue(orgName);
		// 设置单元格样式
		orgNameCell.setCellStyle(titleCellStyle);

		HSSFCell yearCell = titleRow.createCell(3);
		// 设置标题文本
		yearCell.setCellValue(date);
		// 设置单元格样式
		yearCell.setCellStyle(titleCellStyle);
		...
		HSSFRow countRow = hssfSheet.createRow(1);
		HSSFCell areaSum = countRow.createCell(0);
		areaSum.setCellValue(new HSSFRichTextString("使用人数"));
		areaSum.setCellStyle(titleCellStyle);
		...
		HSSFCell areaUseTotle = countRow.createCell(2);
		areaUseTotle.setCellValue(new HSSFRichTextString("使用次数"));
		areaUseTotle.setCellStyle(titleCellStyle);
	...
	public static HSSFWorkbook createExcel2ExportByAreaUseActivity(String sheetName, String title, String[] headers,
			List<?> detailDataList, String date) throws Exception {
		if (ArrayUtils.isEmpty(headers)) {
			LOGGER.warn("【参数headers为空】");
			throw new IllegalArgumentException("headers");
		...
		Long useTakingTotal = new Long(0); // 总时长
		int orgTotal = 0;// 机构数量
		for (Object bean : detailDataList) {
			if (bean instanceof AreaMonthUseTakingCount) {
				AreaMonthUseTakingCount count = (AreaMonthUseTakingCount) bean;

				if (count.getOrgNum() != null) {
					orgTotal = orgTotal + Integer.valueOf(count.getOrgNum());// 学校数量，冗余字段
				}
				useTakingTotal = useTakingTotal + count.getUseTaking();
		...
		HSSFCell areaUseTotleValue = countRow.createCell(3);
		areaUseTotleValue.setCellValue(new HSSFRichTextString(CommonFunction.formatDuring(new Long(useTakingTotal))));
		areaUseTotleValue.setCellStyle(titleCellStyle);
	...
	public static HSSFWorkbook createExcel2ExportByOrgUseActivity(String sheetName, String provinceName,
			String cityName, String[] headers, List<?> detailDataList, String date) throws Exception {
		...
		LOGGER.info("【生成Excel,用于导出】sheetName : " + sheetName + "  , headers : " + Arrays.toString(headers));

		Long loginTakingTotal = new Long(0); // 总时长
		int useTotal = 0;// 机构数量
		for (Object bean : detailDataList) {
			if (bean instanceof OrgMonthUseTakingCount) {
				OrgMonthUseTakingCount count = (OrgMonthUseTakingCount) bean;

				if (count.getUserNum() != null) {
					useTotal = useTotal + Integer.valueOf(count.getUserNum());// 学校数量，冗余字段
				}

				loginTakingTotal = loginTakingTotal + count.getUseTaking();
			}
		}
		...
		HSSFCell areaUseTotle = countRow.createCell(2);
		areaUseTotle.setCellValue(new HSSFRichTextString("总时长"));
		areaUseTotle.setCellStyle(titleCellStyle);

		HSSFCell areaUseTotleValue = countRow.createCell(3);
		areaUseTotleValue.setCellValue(new HSSFRichTextString(CommonFunction.formatDuring(new Long(loginTakingTotal))));
		areaUseTotleValue.setCellStyle(titleCellStyle);
	...
	public static HSSFWorkbook createExcel2ExportByUseActivity(String sheetName, String provinceName, String cityName,
			String orgName, String[] headers, List<?> detailDataList, String date) throws Exception {
		if (ArrayUtils.isEmpty(headers)) {
			LOGGER.warn("【参数headers为空】");
			throw new IllegalArgumentException("headers");
		...
		Long useTakingTotal = new Long(0);

		for (Object bean : detailDataList) {
			if (bean instanceof UserMonthUseTakingCount) {
				UserMonthUseTakingCount count = (UserMonthUseTakingCount) bean;
		...
		HSSFCell areaUseTotleValue = countRow.createCell(3);
		areaUseTotleValue.setCellValue(new HSSFRichTextString(CommonFunction.formatDuring(useTakingTotal)));
		areaUseTotleValue.setCellStyle(titleCellStyle);
