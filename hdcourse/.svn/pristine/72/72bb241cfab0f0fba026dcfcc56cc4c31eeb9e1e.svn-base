...
<div class="mg15 txq_main">
	<form id="pageForm"  name="pageForm" action="${ctx}/manage/overseasLicenceCode/list" method="post">
		...
		<table class="small_space data_list" width="100%" style="border-collapse:collapse;">
		    ...
		    <c:forEach items="${licenceCodeList}" var="g" varStatus="status">
			    <tr>
					...
					<td>
						<c:if test="${!empty g.end_time}">未绑定
	...
	<div class="base_page clearfix">
		<!-- 分页 -->
		<c:if test="${not emhttp://localhost:8080/hdcourse/manage/licenceOrg/listpty page.totalPage && page.totalPage >= 1 }">
...
function cancel(){
	...
	$.confirm("确定要注销吗?注销后将不能恢复！",function(){
		$.ajax({
			type : "POST",
			url : "${ctx}/manage/overseasLicenceCode/cancel",
			data : {"ids" : ids},
			async : false,
			dataType : 'text',
...
function edit(id){
	...
	$.ajax({
		type : "POST",
		url : "${ctx}/manage/overseasSoftLicence/detailEbpSoftLicence",
		data : {"id" : id},
		async : false,
		dataType : 'json',
...
//		}
		...
		setTimeout(function(){
			$.ajax({
				type : "POST",
				url : "${ctx}/manage/overseasSoftLicence/saveEbpSoftLicence",
				data : {
