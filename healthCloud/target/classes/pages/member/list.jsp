			...
			<form id="subForm" class="form-inline">
				<ol id="subDiv" class="breadcrumb">
					...
					<div class="form-group">
						...
						<input type="text" class="form-control" style="width: 180px;" id="identity_card" name="identity_card" value="${identity_card }" placeholder="请输入身份证号查询" >
					</div>
					&nbsp;&nbsp;
						...
						<tbody>
							<c:forEach var="member" items="${member_list}" varStatus="status">
								<tr>
									...
									<td><a href="org_relation?memberId=${member.memberId }">查看</a></td>
									<td><a href="deviceList?memberId=${member.memberId }">设备详情</a></td>
									<td><a href="memberInformation?memberId=${member.memberId }">会员详情</a></td>
									<td>
										<a class="btn btn-info btn-xs" href="toEdit?memberId=${member.memberId }">编辑</a>
										<a class="btn btn-warning btn-xs" href="javascript:;" onclick="disabled(${member.memberId })">禁用</a>
									</td>
								</tr>
							</c:forEach>
						</tbody>
...
<script type="text/javascript">
	...
	function disabled(memberId){
 		layer.msg('该操作将使会员账号禁用，是否继续?', {time: 0, btn: ['继续', '点错了'], yes: function(index){
 			layer.close(index);
	        $.ajax({
	        	type: "post",
	            url: "disabled",
