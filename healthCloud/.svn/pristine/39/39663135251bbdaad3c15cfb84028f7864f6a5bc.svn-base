		...
		<div class="row-fluid" >
			...
			<div id="right" style="padding-left: 0px; padding-right: 0px;">
				<ul class="nav nav-tabs">
					<li class="active"><a href="list">员工列表</a></li>
					<li><a href="edit">添加员工</a></li>
				</ul>
				...
				<div class="col-md-12" id="modalBodyList" style="padding-right: 0px; padding-left: 0px;">
					...
					<form id="subForm" class="form-inline">
						...
						&nbsp;&nbsp;
						<button type="button" class="btn btn-info" onclick="exportExcel('subForm','export')">批量导出</a>
					</form>
					</ol>
				</div>
...
<script type="text/javascript">
	...
	var allColumns = [{
		...
		"data" : "",
		"render" : function(data, type, row, meta) {
			var htmlStr1 = "<a class='btn btn-info btn-xs' data-toggle='modal' data-target='#management' href='management?empId="+row.empId+"'>分配会员</a>";
			var htmlStr2 = "<a class='btn btn-info btn-xs' href='edit?empId="+row.empId+"'>编辑</a>";
			var htmlStr3 = "<a class='btn btn-warning btn-xs' href='javascript:;' onclick='resetPwd("+row.empId+")'>重置密码</a>";
			var htmlStr4 = "<a class='btn btn-danger btn-xs' href='javascript:;' onclick='disabled("+row.empId+")'>禁用</a>";
	...
	function resetPwd(empId){
		layer.msg('该操作将重置员工的密码为初始值，是否继续?', { time: 0, btn: ['继续', '点错了'], yes: function(index){
			layer.close(index);
           	$.ajax({
           		type: "post",
               	url: "resetPwd",
