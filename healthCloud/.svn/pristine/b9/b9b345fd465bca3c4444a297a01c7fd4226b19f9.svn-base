...
<table id="dataTable" class="table table-hover table-bordered" style="margin-top: 10px;">
	<thead>
		<tr>
			<th style="width: 12%;">服务供应商名称</th>
			<th style="width: 10%;">所属机构</th>
			<th style="width: 10%;">供应商类别</th>
			<th style="width: 12%;">供应商类型</th>
			<th style="width: 15%;">归属区域</th>
	...
	<tbody>
		...
		<c:forEach items="${providers }" var="provider">
			<tr>
				...
				<td>${provider.org.orgName }</td>
				<td>${provider.providerCategoryView }</td>
				<td>${provider.providerTypeView }</td>
				<td>${provider.area }</td>
				<td>${provider.linkman }</td>
				<td>${provider.phoneNumber }</td>
				<td>${provider.tel }</td>
				<td>
					<a class="btn btn-primary btn-xs" href="toEdit?providerId=${provider.providerId }">编辑</a>
					<button type="button" class="btn btn-warning btn-xs" onclick="javascript:toDelete(${provider.providerId });">删除</button>
				</td>
			</tr>
		</c:forEach>
	</tbody>
...
function toDelete(providerId){
	//询问框
	layer.confirm('您确认要删除此服务供应商吗？', {
	  btn: ['确认','取消'] //按钮
	}, function(){
		window.location.href = "delete?providerId=" + providerId + "&orgId=" + orgId;
	}, function(){
	  return;
	});
}
...
$(document).ready(function() {
	...
	$(".col-sm-6").eq(0).wrapInner( '<a class="btn btn-primary btn-sm" href="toAdd.do?orgId=${orgId}">新增服务供应商</a>');
