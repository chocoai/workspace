	...
	<tbody>
		<c:forEach items="${contentList }" var="content">
			<tr>
				...
				<td style="overflow:hidden;white-space:nowrap;text-overflow:ellipsis;" title="${content.title}">${content.title}</td>
				...
				<td>
					<button type="button" class="btn btn-primary btn-xs" onclick="javascript:viewDetail(${content.contentId });">查看</button>
					<button type="button" class="btn btn-info btn-xs" onclick="javascript:cancelArchive(${content.contentId });">取消归档</button>
				</td>
			</tr>
		</c:forEach>
	</tbody>
...
<script type="text/javascript">
	...
	function cancelArchive(contentId){
		//询问框
		layer.confirm('您确认要取消归档此内容吗？', { btn: ['确认','取消'] }, 
			function(){
				// window.location.href = "cancelArchive?contentId=" + contentId;
				$.ajax({
					async : false,
					cache : false,
					contentType : false,
					...
					processData : false,
					type : "get",
					url : "cancelArchive?contentId=" + contentId,
	...
	$(document).ready(function() {
		...
		$(".col-sm-6").eq(0).wrapInner('<button class="btn btn-info btn-sm" id="batchCancleArchive">批量取消归档</button>');
		
		$("#batchCancleArchive").click(function(){
			var contentIds = new Array()
			$("input[type='checkbox'][name='check_content']:checked").each(function(i) {
				contentIds[i] = $(this).val();
			});
			if(contentIds.length == 0){
				layer.msg('请先选取要取消归档的内容', { anim: 6 });
				return;
			}
			
			layer.confirm('您确认要取消归档这些内容吗？', { btn: ['确认','取消'] }, 
				function(){
					$.ajax({
						cache : true,
						type : "post",
						url : 'batchCancleArchive',
						async : false,
						data : {
						...
						traditional : true,
						success : function(data) {
							layer.msg('取消归档成功', { anim: 6 });
							// window.location.reload();
							getArchived();
						},
						error : function() {
							layer.msg('取消归档失败', { anim: 6 });
						}
					})
