...
<div class="wql_g_content wql_bge7e9eb pdb30">
    <div class="w1200">
        ...
        <div class="wql_g_mbx">
            <div class="wql_mbx01">
                <span class="prev"><a href="" class="mgr5">用户管理</a>&gt;</span><span class="on mgl5">教师信息管理</span>
            </div>
        </div>
        <!-- 面包屑 E-->
...
function loadList(){
	$.ajax({
		type : "POST",
		url : "${ctx}/manage/teacher/querySchool",
		data : {
			"areaCode" : areaCode,
		...
		success : function(data) {
			...
			if(htmlStr != ''){
				$("#teacherList").html(htmlStr);
			}else{
				$("#teacherList").html('<tr><td colspan="12" align="center"><font color="red">暂无数据</font></td></tr>');
				$("#base_page").html('');
			}
		}
	});
