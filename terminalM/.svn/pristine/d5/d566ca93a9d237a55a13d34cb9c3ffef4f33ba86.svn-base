 ...
 * @date 2018年6月18日
 */
public class LockScreenCommandServiceImpl implements SendDeviceCommandService {
		...
		try {
			...
			if (registerDeviceResponse != null && registerDeviceResponse.getSuccess() != false) {
				...
				if ("online".equals(status.toLowerCase())) {
					...
					invokeThingServiceRequest.setProductKey(bean.getAliProductKey());
					...
					deviceCommandLog.setUpdateTime(new Date());
