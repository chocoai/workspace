...
$(function(){
	$("#submitChangePwd").click(function(){
		...
		var account = '${sessionScope.manageUser.account}';
		var password = '${sessionScope.manageUser.password}';
		var id = '${sessionScope.manageUser.id}';
		if(oldPassword == ''){
		...
		$.ajax({
			type: "POST",
			url: "/terminalM/manageUser/changePassword",
