                    ...
                    <div class="wql_g_nav fl mgt10">
                        <div class="wql_nav01">
                            <ul class="clearfix">
                                ...
                                <li class="wql_li on"><span class="wql_bl"></span><a href="${ctx}/manage/appWhitelistRule/list">应用白名单</a></li>
	 ...
	 var dialog = art.dialog({
         ...
         ok:function(){
        	var name = $("input[name='name']").val();//分类名称
         	...
         	var appList = $("#appList").children("tr");
         	
         	var appIdList =[];
         	
         	appList.each(function(){ //便利除标题行外所有行        			
         		var appId = $(this).find("input[name='tdAppId']").val();
         	...
         	$.ajax({
         		type : "POST",
         		url : "${ctx}/manage/appWhitelistRule/updateAppWhitelistRule",
         		traditional:true,
         		data :{
         			"name" : name,
         			"memo" : memo,
...
function deleteRule(id){
	...
	myConfirm('系统确认框','是否删除！',function(r){
		 if(r){  
			 $.ajax({
					type : "POST",
					url : "${ctx}/manage/appWhitelistRule/deleteAppWhitelistRule",
					traditional:true,
					data :{
						"id" : id
					},
    ...
    $('.wql_addBtn').on('click',function(){
        ...
        var dialog = art.dialog({
            ...
            ok:function(){
            	...
            	$.ajax({
            		type : "POST",
            		url : "${ctx}/manage/appWhitelistRule/saveAppWhitelistRule",
            		traditional:true,
            		data :{
            			"name" : name,
            			"memo" : memo,
