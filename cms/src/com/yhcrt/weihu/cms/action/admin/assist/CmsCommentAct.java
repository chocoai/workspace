...
public class CmsCommentAct {
	...
	public String list(Boolean queryChecked,
			Boolean queryRecommend, Integer pageNo, HttpServletRequest request,
			ModelMap model) {
		CmsSite site = CmsUtils.getSite(request);
		Pagination pagination = manager.getPage(site.getId(), null,
...
//		model.addAttribute("queryContentId",queryContentId);
	...
	public String update(Integer queryContentId, Boolean queryChecked,Integer queryBaoLiaoId,
		...
		cmsLogMng.operating(request, "cmsComment.log.update", "id="
				+ bean.getId());
		return list(queryChecked, queryRecommend, pageNo,
				request, model);
	}

	@RequiresPermissions("comment:o_delete")
	@RequestMapping("/comment/o_delete.do")
	public String delete(Integer queryContentId, Integer queryBaoLiaoId,Boolean queryChecked,
		...
		for (CmsComment bean : beans) {
			if(baoliao.getCommtCount()>1){
				baoliao.setCommtCount(baoliao.getCommtCount()-1);
			}else{
				baoliao.setCommtCount(0);
			}
	...
	public String check(Integer queryContentId, Integer queryBaoLiaoId,Boolean queryRecommend,
		...
		return list(queryRecommend, queryChecked, pageNo, request,
				model);
	}
	
	@RequiresPermissions("comment:o_check_cancel")
	@RequestMapping("/comment/o_check_cancel.do")
	public String cancelCheck(Integer queryContentId, Integer queryBaoLiaoId,Boolean queryRecommend,
			Boolean queryChecked, Integer[] ids, Integer pageNo,
			HttpServletRequest request, ModelMap model) {
		WebErrors errors = validateDelete(ids, request);
		if (errors.hasErrors()) {
