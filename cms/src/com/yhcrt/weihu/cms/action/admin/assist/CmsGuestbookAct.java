...
public class CmsGuestbookAct {
	...
	private Integer[] getDepartmentCtgIds(Integer departId){
		Set<Integer>ctgIds=new HashSet<Integer>();
		Integer[]departCtgIds;
		departCtgIds=departmentMng.findById(departId).getGuestBookCtgIds();
		if(departCtgIds!=null){
			for(Integer dcid:departCtgIds){
				ctgIds.add(dcid);
			}
		}
		Integer[]results=new Integer[ctgIds.size()];
		Object[]objects=ctgIds.toArray();
