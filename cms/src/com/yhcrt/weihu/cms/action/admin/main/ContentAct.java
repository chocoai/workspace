...
import com.yhcrt.weihu.cms.entity.main.CmsModelItem;
...
import com.yhcrt.weihu.cms.entity.main.ContentTopic;
...
import com.yhcrt.weihu.cms.manager.main.CmsModelItemMng;
...
import com.yhcrt.weihu.cms.service.ImageSvc;
import com.yhcrt.weihu.cms.staticpage.exception.ContentNotCheckedException;
import com.yhcrt.weihu.cms.staticpage.exception.GeneratedZeroStaticPageException;
import com.yhcrt.weihu.cms.staticpage.exception.StaticPageNotOpenException;
import com.yhcrt.weihu.cms.staticpage.exception.TemplateNotFoundException;
import com.yhcrt.weihu.cms.staticpage.exception.TemplateParseException;
import com.yhcrt.weihu.common.image.ImageUtils;
import com.yhcrt.weihu.common.office.FileUtils;
import com.yhcrt.weihu.common.office.OpenOfficeConverter;
import com.yhcrt.weihu.common.page.Pagination;
import com.yhcrt.weihu.common.upload.FileRepository;
import com.yhcrt.weihu.common.upload.UploadUtils;
...
import com.yhcrt.weihu.core.entity.CmsGroup;
...
import com.yhcrt.weihu.core.manager.CmsGroupMng;
...
import com.yhcrt.weihu.core.manager.DbFileMng;
...
public class ContentAct {
    ...
    public String list(String queryStatus, Integer queryTypeId, Boolean queryTopLevel, Boolean queryRecommend,
	    Integer queryOrderBy, Integer cid, Integer pageNo, HttpServletRequest request, ModelMap model) {
	...
	Pagination p = manager.getPageByRight(queryTitle, queryTypeId, user.getId(), queryInputUserId, queryTopLevel,
		queryRecommend, status, user.getCheckStep(siteId), siteId, cid, userId, queryOrderBy, cpn(pageNo),
	...
	addAttibuteForQuery(model, queryTitle, queryInputUsername, queryStatus, queryTypeId, queryTopLevel,
		queryRecommend, queryOrderBy, pageNo);
    ...
    public String view(String queryStatus, Integer queryTypeId, Boolean queryTopLevel, Boolean queryRecommend,
	    Integer queryOrderBy, Integer pageNo, Integer cid, Integer id, HttpServletRequest request, ModelMap model) {
	CmsSite site = CmsUtils.getSite(request);
	CmsUser user = CmsUtils.getUser(request);
    ...
    public String edit(String queryStatus, Integer queryTypeId, Boolean queryTopLevel, Boolean queryRecommend,
	    Integer queryOrderBy, Integer pageNo, Integer cid, Integer id, HttpServletRequest request, ModelMap model) {
	...
	List<ContentTopic> contentTopics = content.getContentTopic();
	Integer[] topicIds = new Integer[contentTopics.size()];
	Integer[] topicChannelIds = new Integer[contentTopics.size()];
	Integer topicId = 0;
	
	for (int i = 0 ;i<contentTopics.size();i++) {
	    topicIds[i] = contentTopics.get(i).getTopicId();
	    topicChannelIds[i] =  contentTopics.get(i).getTopicChannelId();
	    topicId = contentTopics.get(i).getTopicId();
	    CmsTopic topic = cmsTopicMng.findById(contentTopics.get(i).getTopicId());
	    if (topic != null && !topicList.contains(topic)) {
		topicList.add(topic);
	    }
	}
	// 根据专题Id查询专题的子栏目列表
	List<TopicChannel> topicChannels = topicChannelMng.getListByTopic(topicId);
	...
	model.addAttribute("topicChannelIds", topicChannelIds);
    ...
    public String save(Content bean, ContentExt ext, ContentTxt txt, ContentDoc doc, Boolean copyimg,
	    Integer[] channelIds, Integer topicId, Integer topicChannelId, Integer[] viewGroupIds, String[] attachmentPaths,
	    String[] attachmentNames, String[] attachmentFilenames, String[] picPaths, String[] picDescs,
	    Integer channelId, Integer typeId, String tagStr, Boolean draft, Integer cid, Integer modelId,
	...
	String[] tagArr = StrUtils.splitAndTrim(tagStr, ",",
		MessageResolver.getMessage(request, "content.tagStr.split"));
	if (txt != null && copyimg != null && copyimg) {
	    txt = copyContentTxtImg(txt, site);
	}
	bean = manager.save(bean, ext, txt, doc, channelIds, topicId, topicChannelId, viewGroupIds, tagArr, attachmentPaths,
		attachmentNames, attachmentFilenames, picPaths, picDescs, channelId, typeId, draft, false, user, false);
	// 处理附件
	fileMng.updateFileByPaths(attachmentPaths, picPaths, ext.getMediaPath(), ext.getTitleImg(), ext.getTypeImg(),
		ext.getContentImg(), true, bean);
    ...
    public String update(String queryStatus, Integer queryTypeId, Boolean queryTopLevel, Boolean queryRecommend,
	    Integer queryOrderBy, Content bean, ContentExt ext, ContentTxt txt, ContentDoc doc, Boolean copyimg,
	    Integer[] channelIds, Integer topicId, Integer topicChannelId, Integer[] viewGroupIds, String[] attachmentPaths,
	    String[] attachmentNames, String[] attachmentFilenames, String[] picPaths, String[] picDescs,
	    Integer channelId, Integer typeId, String tagStr, Boolean draft, Integer cid, String[] oldattachmentPaths,
	    String[] oldpicPaths, String oldTitleImg, String oldContentImg, String oldTypeImg, Integer pageNo,
	    HttpServletRequest request, ModelMap model) {
	...
	bean = manager.update(bean, ext, txt, doc, tagArr, channelIds, topicId,topicChannelId, viewGroupIds, attachmentPaths,
		attachmentNames, attachmentFilenames, picPaths, picDescs, attr, channelId, typeId, draft, user, false);
	// 处理之前的附件有效性
	fileMng.updateFileByPaths(oldattachmentPaths, oldpicPaths, null, oldTitleImg, oldTypeImg, oldContentImg, false,
		bean);
	// 处理更新后的附件有效性
	fileMng.updateFileByPaths(attachmentPaths, picPaths, ext.getMediaPath(), ext.getTitleImg(), ext.getTypeImg(),
		ext.getContentImg(), true, bean);
    ...
    public String delete(String queryStatus, Integer queryTypeId, Boolean queryTopLevel, Boolean queryRecommend,
	    Integer queryOrderBy, Integer[] ids, Integer cid, Integer pageNo, HttpServletRequest request,
	    ModelMap model) {
	CmsSite site = CmsUtils.getSite(request);
    ...
    public String check(String queryStatus, Integer queryTypeId, Boolean queryTopLevel, Boolean queryRecommend,
	    Integer queryOrderBy, Integer[] ids, Integer cid, Integer pageNo, HttpServletRequest request,
	    ModelMap model) {
    ...
    public void ajaxCheck(Integer[] ids, HttpServletRequest request, HttpServletResponse response, ModelMap model)
    ...
    public String contentStatic(String queryStatus, Integer queryTypeId, Boolean queryTopLevel, Boolean queryRecommend,
	    Integer queryOrderBy, Integer[] ids, Integer cid, Integer pageNo, HttpServletRequest request,
	    ModelMap model) {
	...
	try {
	    ...
	    model.addAttribute("message",
		    errors.getMessage(e.getMessage(), new Object[] { e.getErrorTitle(), e.getGenerated() }));
    ...
    public String reject(String queryStatus, Integer queryTypeId, Boolean queryTopLevel, Boolean queryRecommend,
	    Integer queryOrderBy, Integer[] ids, Integer cid, String rejectOpinion, Integer pageNo,
	    HttpServletRequest request, ModelMap model) {
    ...
    public void ajaxReject(Integer[] ids, String rejectOpinion, HttpServletRequest request,
    ...
    public String submit(String queryStatus, Integer queryTypeId, Boolean queryTopLevel, Boolean queryRecommend,
	    Integer queryOrderBy, Integer[] ids, Integer cid, Integer pageNo, HttpServletRequest request,
	    ModelMap model) {
	WebErrors errors = validateCheck(ids, request);
	if (errors.hasErrors()) {
    ...
    public void copy(Integer contentIds[], Integer channelId, Integer siteId, HttpServletRequest request,
	...
	if (contentIds != null) {
		...
		manager.save(beanCopy, extCopy, txtCopy, docCopy, null, null,null, bean.getViewGroupIds(),
			bean.getTagArray(), bean.getAttachmentPaths(), bean.getAttachmentNames(),
			bean.getAttachmentFileNames(), bean.getPicPaths(), bean.getPicDescs(), channelId,
			bean.getType().getId(), draft, false, user, false);
	    }
	}
	json.put("pass", pass);
	ResponseUtils.renderJson(response, json.toString());
    ...
    public void contentCopy(Integer contentIds[], Integer channelId, Integer siteId, HttpServletRequest request,
	    HttpServletResponse response) throws JSONException {
    ...
    public void refer(Integer contentIds[], Integer channelId, HttpServletRequest request, HttpServletResponse response)
	    throws JSONException {
	JSONObject json = new JSONObject();
	CmsUser user = CmsUtils.getUser(request);
	Boolean pass = true;
    ...
    public String priority(Integer[] wids, Byte[] topLevel, String queryStatus, Integer queryTypeId,
	    Boolean queryTopLevel, Boolean queryRecommend, Integer queryOrderBy, Integer cid, Integer pageNo,
	    HttpServletRequest request, ModelMap model) {
    ...
    public String pigeonhole(Integer[] ids, String queryStatus, Integer queryTypeId, Boolean queryTopLevel,
	    Boolean queryRecommend, Integer queryOrderBy, Integer cid, Integer pageNo, HttpServletRequest request,
	    ModelMap model) {
    ...
    public String unpigeonhole(Integer[] ids, String queryStatus, Integer queryTypeId, Boolean queryTopLevel,
	    Boolean queryRecommend, Integer queryOrderBy, Integer cid, Integer pageNo, HttpServletRequest request,
	    ModelMap model) {
	for (int i = 0; i < ids.length; i++) {
	    Content c = manager.findById(ids[i]);
    ...
    public void sendToTopic(Integer contentIds[], Integer topicId,Integer topicChannelId, HttpServletRequest request,
	    HttpServletResponse response) throws JSONException {
	JSONObject json = new JSONObject();
	CmsUser user = CmsUtils.getUser(request);
	Boolean pass = true;
	...
	if (contentIds != null) {
		...
		manager.addContentToTopics(contentId, topicId, topicChannelId);
	    }
	} else {
	    ResponseUtils.renderJson(response, "false");
	}
    ...
    public String uploadAttachment(@RequestParam(value = "attachmentFile", required = false) MultipartFile file,
    ...
    public String uploadMedia(@RequestParam(value = "mediaFile", required = false) MultipartFile file, String filename,
	    HttpServletRequest request, ModelMap model) {
	CmsSite site = CmsUtils.getSite(request);
	CmsUser user = CmsUtils.getUser(request);
	String origName = file.getOriginalFilename();
    ...
    public String uploadDoc(@RequestParam(value = "docFile", required = false) MultipartFile file, String filename,
	    HttpServletRequest request, ModelMap model) {
	CmsSite site = CmsUtils.getSite(request);
	CmsUser user = CmsUtils.getUser(request);
	String origName = file.getOriginalFilename();
    ...
    public String cycleList(Integer queryTypeId, Boolean queryTopLevel, Boolean queryRecommend, Integer queryOrderBy,
	    Integer cid, Integer pageNo, HttpServletRequest request, ModelMap model) {
	list(ContentStatus.recycle.toString(), queryTypeId, queryTopLevel, queryRecommend, queryOrderBy, cid, pageNo,
		request, model);
    ...
    public String cycleRecycle(String queryStatus, Integer queryTypeId, Boolean queryTopLevel, Boolean queryRecommend,
	    Integer queryOrderBy, Integer[] ids, Integer cid, Integer pageNo, HttpServletRequest request,
	    ModelMap model) {
	WebErrors errors = validateDelete(ids, request);
	if (errors.hasErrors()) {
    ...
    public String cycleDelete(String queryStatus, Integer queryTypeId, Boolean queryTopLevel, Boolean queryRecommend,
	    Integer queryOrderBy, Integer[] ids, Integer cid, Integer pageNo, HttpServletRequest request,
	    ModelMap model) {
	CmsSite site = CmsUtils.getSite(request);
	WebErrors errors = validateDelete(ids, request);
    ...
    public String uploadDocs(@RequestParam(value = "docFile", required = false) MultipartFile file, String docNum,
	    HttpServletRequest request, ModelMap model) {
	CmsSite site = CmsUtils.getSite(request);
	CmsUser user = CmsUtils.getUser(request);
	String origName = file.getOriginalFilename();
    ...
    public String submitDocs(String[] docPaths, String[] docNames, String[] docFilenames, Integer channelId,
	    Integer typeId, Integer modelId, Boolean draft, HttpServletRequest request, ModelMap model) {
	if (docPaths != null && docPaths.length > 0) {
	    ...
	    String docImgPath = request.getContextPath() + CmsUtils.getSite(request).getUploadPath() + "/"
    ...
    private void saveContent(Integer channelId, Integer typeId, Integer modelId, Boolean draft, String title,
	    String txt, HttpServletRequest request) {
    ...
    private void addAttibuteForQuery(ModelMap model, String queryTitle, String queryInputUsername, String queryStatus,
	    Integer queryTypeId, Boolean queryTopLevel, Boolean queryRecommend, Integer queryOrderBy, Integer pageNo) {
	...
	if (bean.getModel().getId() != null) {
	    ...
	    // 默认没有配置的情况下modelIds为空 则允许添加
	    if (channel.getModelIds().size() > 0
		    && !channel.getModelIds().contains(bean.getModel().getId().toString())) {
