...
import static com.yhcrt.weihu.cms.Constants.TPLDIR_INDEX;
...
import com.yhcrt.weihu.cms.manager.assist.CmsKeywordMng;
...
import com.yhcrt.weihu.common.page.Paginable;
import com.yhcrt.weihu.common.page.SimplePage;
...
import com.yhcrt.weihu.core.manager.CmsConfigMng;
import com.yhcrt.weihu.core.web.util.CmsUtils;
import com.yhcrt.weihu.core.web.util.FrontUtils;
import com.yhcrt.weihu.core.web.util.URLHelper;
import com.yhcrt.weihu.core.web.util.URLHelper.PageInfo;
...
public class DynamicPageAct {
    ...
    public static final String TPL_TOPS = "tpl.tops";

    public static final String TPL_JDS = "tpl.jds";
	...
	model.put("bbsTopics", bbsTopicMng.getList(10));
	// 使用静态首页而且静态首页存在
     ...
     * @return
     */
    @RequestMapping(value = "/tops*.jhtml", method = RequestMethod.GET)
    public String tops(HttpServletRequest request, HttpServletResponse response, ModelMap model) {

	CmsSite site = CmsUtils.getSite(request);
	FrontUtils.frontPageData(request, model);
	FrontUtils.frontData(request, model, site);
	return FrontUtils.getTplPath(request, site.getSolutionPath(), TPLDIR_INDEX, TPL_TOPS);
     ...
     * @return
     */
    @RequestMapping(value = "/jds*.jhtml", method = RequestMethod.GET)
    public String jds(HttpServletRequest request, HttpServletResponse response, ModelMap model) {

	CmsSite site = CmsUtils.getSite(request);
	FrontUtils.frontPageData(request, model);
	FrontUtils.frontData(request, model, site);
	return FrontUtils.getTplPath(request, site.getSolutionPath(), TPLDIR_INDEX, TPL_JDS);
	...
	if (len == 2) {
		...
		// 单页
		return channel(paths[1], true, pageNo, params, info, request, response, model);
	    }
	} else if (len == 3) {
	    if (paths[2].equals(INDEX)) {
		// 栏目页
		return channel(paths[1], false, pageNo, params, info, request, response, model);
	    } else {
		// 内容页
		try {
	...
	if (len == 1) {
	    // 单页
	    return channel(paths[0], true, pageNo, params, info, request, response, model);
	} else if (len == 2) {
	    if (paths[1].equals(INDEX)) {
		// 栏目页
		return channel(paths[0], false, pageNo, params, info, request, response, model);
	    } else {
		// 内容页
		try {
    ...
    public String channel(String path, boolean checkAlone, int pageNo, String[] params, PageInfo info,
    ...
    public String content(Integer id, int pageNo, String[] params, PageInfo info, HttpServletRequest request,
	    HttpServletResponse response, ModelMap model) {
