...
import com.yhcrt.weihu.cms.manager.assist.CmsWebserviceMng;
...
public class RegisterAct {
    ...
    public static final String REGISTER_TERMS = "tpl.registerTerms";

    public static final String REGISTER_ACTIVE_SUCCESS = "tpl.registerActiveSuccess";

    public static final String LOGIN_INPUT = "tpl.loginInput";

    @RequestMapping(value = "/terms.jspx", method = RequestMethod.GET)
    public String terms(HttpServletRequest request, HttpServletResponse response, ModelMap model) {
	CmsSite site = CmsUtils.getSite(request);
	FrontUtils.frontData(request, model, site);
	return FrontUtils.getTplPath(request, site.getSolutionPath(), TPLDIR_MEMBER, REGISTER_TERMS);
    }
	...
	List<CmsConfigItem> items = cmsConfigItemMng.getList(site.getConfig().getId(),
		CmsConfigItem.CATEGORY_REGISTER);
	FrontUtils.frontData(request, model, site);
    ...
    public String submit(String username, String email, String password, CmsUserExt userExt,
	    String captcha, String nextUrl, HttpServletRequest request,
	    HttpServletResponse response, ModelMap model) throws IOException {
	CmsSite site = CmsUtils.getSite(request);
	CmsConfig config = site.getConfig();
	WebErrors errors = validateSubmit(username, email, password, captcha, site, request,
		response);
	...
	if (config.getEmailValidate()) {
		...
		try {
		    cmsUserMng.registerMember(username, email, password, ip, null, disabled,
			    userExt, attrs, false, sender, msgTpl);
		    cmsWebserviceMng.callWebService("false", username, password, email, userExt,
	...
	FrontUtils.frontData(request, model, site);
	return FrontUtils.getTplPath(request, site.getSolutionPath(), TPLDIR_MEMBER,
	...
	if(errors.ifNotUsername(username, "username", 3, 16)){
	    ResponseUtils.renderJson(response, "formatError");
	    return;
	}
    ...
    private WebErrors validateSubmit(String username, String email, String password, String captcha,
