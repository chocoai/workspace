...
public abstract class AbstractContentDirective implements TemplateDirectiveModel {
     ...
     * 输入参数，专题的子栏目ID。
     */
    public static final String PARAM_TOPIC_CHANNEL_ID = "topicChannelId";
     ...
     * @throws TemplateException
     */
    protected Integer getTopicChannelId(Map<String, TemplateModel> params) throws TemplateException {
	return DirectiveUtils.getInt(PARAM_TOPIC_CHANNEL_ID, params);
    }
     ...
     * @throws TemplateException
     */
    protected Map<String, String[]> getAttrMap(Map<String, TemplateModel> params) throws TemplateException {
     ...
     * @throws TemplateException
     */
    protected int getOpen(Map<String, TemplateModel> params) throws TemplateException {
	...
	if (tagIds != null) {
		...
		return contentMng.getPageByTagIdsForTag(tagIds, siteIds, channelIds, typeIds, excludeId, titleImg,
			recommend, title, open, attr, orderBy, pageNo, count);
	    } else {
		int first = FrontUtils.getFirst(params);
		return contentMng.getListByTagIdsForTag(tagIds, siteIds, channelIds, typeIds, excludeId, titleImg,
	...
	if (topicId != null) {
	    Integer[] channelIds = getChannelIdsOrPaths(params, siteIds);
	    Integer topicChannelId = getTopicChannelId(params);
	    if (isPage()) {
		int pageNo = FrontUtils.getPageNo(env);
		return contentMng.getPageByTopicIdForTag(topicId, topicChannelId, siteIds, channelIds, typeIds, titleImg, recommend,
			title, open, attr, orderBy, pageNo, count);
	    } else {
		int first = FrontUtils.getFirst(params);
		return contentMng.getListByTopicIdForTag(topicId, topicChannelId, siteIds, channelIds, typeIds, titleImg, recommend,
	...
	if (channelIds != null) {
	    ...
	    if (isPage()) {
		int pageNo = FrontUtils.getPageNo(env);
		return contentMng.getPageByChannelIdsForTag(channelIds, typeIds, titleImg, recommend, title, open, attr,
			orderBy, option, pageNo, count);
	    } else {
		int first = FrontUtils.getFirst(params);
		return contentMng.getListByChannelIdsForTag(channelIds, typeIds, titleImg, recommend, title, open, attr,
			orderBy, option, first, count);
	    }
	}
	...
	if (channelPaths != null) {
		...
		if (channelIds != null) {
		    if (isPage()) {
			int pageNo = FrontUtils.getPageNo(env);
			return contentMng.getPageByChannelIdsForTag(channelIds, typeIds, titleImg, recommend, title,
				open, attr, orderBy, option, pageNo, count);
		    } else {
			int first = FrontUtils.getFirst(params);
			return contentMng.getListByChannelIdsForTag(channelIds, typeIds, titleImg, recommend, title,
				open, attr, orderBy, option, first, count);
		    }
		} else {
		...
		if (isPage()) {
		    int pageNo = FrontUtils.getPageNo(env);
		    return contentMng.getPageByChannelPathsForTag(channelPaths, siteIds, typeIds, titleImg, recommend,
			    title, open, attr, orderBy, pageNo, count);
		} else {
		    int first = FrontUtils.getFirst(params);
		    return contentMng.getListByChannelPathsForTag(channelPaths, siteIds, typeIds, titleImg, recommend,
			    title, open, attr, orderBy, first, count);
		}
	    }
	}
	...
	if (isPage()) {
	    int pageNo = FrontUtils.getPageNo(env);
	    return contentMng.getPageBySiteIdsForTag(siteIds, typeIds, titleImg, recommend, title, open, attr, orderBy,
		    pageNo, count);
	} else {
	    int first = FrontUtils.getFirst(params);
	    return contentMng.getListBySiteIdsForTag(siteIds, typeIds, titleImg, recommend, title, open, attr, orderBy,
		    first, count);
	}
    }
