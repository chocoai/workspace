...
import com.yhcrt.weihu.common.upload.FileUpload;
import com.yhcrt.weihu.common.util.PropertyUtils;
...
public class WeiXinSvcImpl implements WeiXinSvc {
	...
	public String getToken() {
		String tokenGetUrl=PropertyUtils.getPropertyValue(new File(realPathResolver.get(Constants.CMS_CONFIG)),TOKEN_KEY);
		String appid="";
		String secret="";
		CmsSite site=CmsThreadVariable.getSite();
		if(site!=null){
			appid=site.getAttr().get(com.yhcrt.weihu.core.Constants.WEIXIN_APPKEY);
			secret=site.getAttr().get(com.yhcrt.weihu.core.Constants.WEIXIN_APPSECRET);
		}
	...
	public Set<String> getUsers(String access_token) {
		String usersGetUrl=PropertyUtils.getPropertyValue(new File(realPathResolver.get(Constants.CMS_CONFIG)),USERS_KEY);
	...
	public String  uploadFile(String access_token,String filePath,String type){
		String sendGetUrl=PropertyUtils.getPropertyValue(new File(realPathResolver.get(Constants.CMS_CONFIG)),UPLOAD_KEY);
	...
	public void sendText(String access_token,String content) {
		String sendGetUrl=PropertyUtils.getPropertyValue(new File(realPathResolver.get(Constants.CMS_CONFIG)),SEND_KEY);
        String url = sendGetUrl+"?access_token=" + access_token;
	...
	public void sendContent(String access_token,String title, String description, String url,
			String picurl) {
		String sendUrl=PropertyUtils.getPropertyValue(new File(realPathResolver.get(Constants.CMS_CONFIG)),SEND_KEY);
	...
	public void createMenu(String menus){
		...
		String createMenuUrl=PropertyUtils.getPropertyValue(new File(realPathResolver.get(Constants.CMS_CONFIG)),MENU_KEY);
	...
	public void sendTextToAllUser(Content[] beans){
		String token=weixinTokenCache.getToken();
		//上传内容到微信
		String articalUploadUrl=PropertyUtils.getPropertyValue(new File(realPathResolver.get(Constants.CMS_CONFIG)),UPLOAD_NEWS);
			...
			DefaultHttpClient client = new DefaultHttpClient();
		       ...
		       try{
		           ...
		           if(StringUtils.isNotBlank(media_id)){
		        	   String sendAllMessageUrl=PropertyUtils.getPropertyValue(new File(realPathResolver.get(Constants.CMS_CONFIG)),SEND_ALL_MESSAGE);
			   		   String url_send = sendAllMessageUrl+"?access_token=" + token;
			   		   String str_send = "{\"filter\":{\"group_id\":\"0\"},\"mpnews\":{\"media_id\":\""+media_id+"\"},\"msgtype\":\"mpnews\"}";
	...
	private String  uploadImg(String access_token,String filePath){
		String sendGetUrl=PropertyUtils.getPropertyValue(new File(realPathResolver.get(Constants.CMS_CONFIG)),UPLOAD_IMG_URL);
        String url = sendGetUrl+"?access_token=" + access_token;
		String result = null;
		String mediaId="";
		FileUpload fileUpload = new FileUpload();
