...
import static com.yhcrt.weihu.cms.entity.assist.CmsSiteAccessStatistic.STATISTIC_AREA;
import static com.yhcrt.weihu.cms.entity.assist.CmsSiteAccessStatistic.STATISTIC_ENGINE;
import static com.yhcrt.weihu.cms.entity.assist.CmsSiteAccessStatistic.STATISTIC_KEYWORD;
import static com.yhcrt.weihu.cms.entity.assist.CmsSiteAccessStatistic.STATISTIC_LINK;
import static com.yhcrt.weihu.cms.entity.assist.CmsSiteAccessStatistic.STATISTIC_SOURCE;
import static com.yhcrt.weihu.cms.entity.assist.CmsSiteAccessStatistic.STATISTIC_TARGET_IP;
import static com.yhcrt.weihu.cms.entity.assist.CmsSiteAccessStatistic.STATISTIC_TARGET_PV;
import static com.yhcrt.weihu.cms.entity.assist.CmsSiteAccessStatistic.STATISTIC_TARGET_VISITORS;
import static com.yhcrt.weihu.cms.entity.assist.CmsSiteAccessStatistic.STATISTIC_TARGET_VISITSECOND;
...
import com.yhcrt.weihu.cms.dao.assist.CmsSiteAccessDao;
import com.yhcrt.weihu.cms.entity.assist.CmsSiteAccess;
 ...
 */
@Repository
public class CmsSiteAccessDaoImpl extends HibernateBaseDao<CmsSiteAccess, Integer> implements CmsSiteAccessDao {
	...
	Finder f = Finder
		.create("select bean.entryPage,sum(bean.visitPageCount),count(distinct bean.sessionId),sum(bean.visitSecond)/sum(bean.visitPageCount) "
			+ "from CmsSiteAccess bean where bean.site.id=:siteId")
	...
	Finder f = Finder.create("from CmsSiteAccess bean where bean.sessionId=:sessionId").setParam("sessionId",
		sessionId);
	...
	Finder f = Finder
		.create("from CmsSiteAccess access where  access.site.id=:siteId and access.accessDate!=:accessDate order by access.accessDate desc ")
	...
	Finder f = Finder
		.create("select sum(access.visitPageCount) as pv,count(distinct access.ip)as ip,count(distinct access.sessionId)as visitors ,sum(access.visitSecond),'' from  CmsSiteAccess access where  access.site.id=:siteId and access.accessDate=:accessDate")
		.setParam("siteId", siteId).setParam("accessDate", date);
	return find(f);
    }
	...
	Finder f = Finder
		.create("select sum(access.visitPageCount) as pv,count(distinct access.ip)as ip,count(distinct access.sessionId)as visitors ,sum(access.visitSecond), access.area from  CmsSiteAccess access where  access.site.id=:siteId and access.accessDate=:accessDate and access.area!='' group by access.area")
		.setParam("siteId", siteId).setParam("accessDate", date);
	return find(f);
    }
	...
	Finder f = Finder
		.create("select sum(access.visitPageCount) as pv,count(distinct access.ip)as ip,count(distinct access.sessionId)as visitors ,sum(access.visitSecond), access.accessSource from  CmsSiteAccess access where  access.site.id=:siteId and access.accessDate=:accessDate  group by access.accessSource")
		.setParam("siteId", siteId).setParam("accessDate", date);
	return find(f);
    }
	...
	Finder f = Finder
		.create("select sum(access.visitPageCount) as pv,count(distinct access.ip)as ip,count(distinct access.sessionId)as visitors ,sum(access.visitSecond), access.engine from  CmsSiteAccess access where  access.site.id=:siteId and access.accessDate=:accessDate and  access.engine!='' group by access.engine")
		.setParam("siteId", siteId).setParam("accessDate", date);
	return find(f);
    }
	...
	Finder f = Finder
		.create("select sum(access.visitPageCount) as pv,count(distinct access.ip)as ip,count(distinct access.sessionId)as visitors ,sum(access.visitSecond), access.externalLink from  CmsSiteAccess access where  access.site.id=:siteId and access.accessDate=:accessDate and  access.externalLink!='' group by access.externalLink")
		.setParam("siteId", siteId).setParam("accessDate", date);
	return find(f);
    }
	...
	Finder f = Finder
		.create("select sum(access.visitPageCount) as pv,count(distinct access.ip)as ip,count(distinct access.sessionId)as visitors ,sum(access.visitSecond), access.keyword from  CmsSiteAccess access where  access.site.id=:siteId and access.accessDate=:accessDate and  access.keyword!='' group by access.keyword")
		.setParam("siteId", siteId).setParam("accessDate", date);
	return find(f);
    }
	...
	Finder f = Finder
		.create("select count(distinct access.sessionId)as visitors ,access.visitPageCount from  CmsSiteAccess access where  access.site.id=:siteId and access.accessDate=:accessDate  group by access.visitPageCount order by count(distinct access.sessionId) desc")
		.setParam("siteId", siteId).setParam("accessDate", date);
	return find(f);
    }
	...
	String hql = "select distinct bean." + property
		+ "  from CmsSiteAccess bean where bean.site.id=:siteId and  bean." + property + " !='' ";
