...
import static com.yhcrt.weihu.cms.action.directive.abs.AbstractContentDirective.PARAM_ATTR_END;
import static com.yhcrt.weihu.cms.action.directive.abs.AbstractContentDirective.PARAM_ATTR_EQ;
import static com.yhcrt.weihu.cms.action.directive.abs.AbstractContentDirective.PARAM_ATTR_GT;
import static com.yhcrt.weihu.cms.action.directive.abs.AbstractContentDirective.PARAM_ATTR_GTE;
import static com.yhcrt.weihu.cms.action.directive.abs.AbstractContentDirective.PARAM_ATTR_IN;
import static com.yhcrt.weihu.cms.action.directive.abs.AbstractContentDirective.PARAM_ATTR_LIKE;
import static com.yhcrt.weihu.cms.action.directive.abs.AbstractContentDirective.PARAM_ATTR_LT;
import static com.yhcrt.weihu.cms.action.directive.abs.AbstractContentDirective.PARAM_ATTR_LTE;
import static com.yhcrt.weihu.cms.action.directive.abs.AbstractContentDirective.PARAM_ATTR_START;
import static com.yhcrt.weihu.cms.entity.main.Content.ContentStatus.all;
import static com.yhcrt.weihu.cms.entity.main.Content.ContentStatus.checked;
import static com.yhcrt.weihu.cms.entity.main.Content.ContentStatus.contribute;
import static com.yhcrt.weihu.cms.entity.main.Content.ContentStatus.draft;
import static com.yhcrt.weihu.cms.entity.main.Content.ContentStatus.passed;
import static com.yhcrt.weihu.cms.entity.main.Content.ContentStatus.pigeonhole;
import static com.yhcrt.weihu.cms.entity.main.Content.ContentStatus.prepared;
import static com.yhcrt.weihu.cms.entity.main.Content.ContentStatus.recycle;
import static com.yhcrt.weihu.cms.entity.main.Content.ContentStatus.rejected;
...
import com.yhcrt.weihu.cms.dao.main.ContentDao;
...
import com.yhcrt.weihu.cms.entity.main.Content.ContentStatus;
...
public class ContentDaoImpl extends HibernateBaseDao<Content, Integer> implements ContentDao {
    public Pagination getPage(String title, Integer typeId, Integer currUserId, Integer inputUserId, boolean topLevel,
	    boolean recommend, ContentStatus status, Byte checkStep, Integer siteId, Integer modelId, Integer channelId,
	    int orderBy, int pageNo, int pageSize) {
	Finder f = Finder.create(
		"select  bean from Content bean left join bean.contentShareCheckSet shareCheck left join shareCheck.channel tarChannel ");
	if (rejected == status) {
	...
	if (channelId != null) {
	    ...
	    f.append(
		    " and parent.id=:parentId)   or (shareCheck.checkStatus<>0 and shareCheck.shareValid=true and  tarChannel.lft between parent.lft and parent.rgt and tarChannel.site.id=parent.site.id and parent.id=:parentId))");
    ...
    public Pagination getPageBySelf(String title, Integer typeId, Integer inputUserId, boolean topLevel,
	    boolean recommend, ContentStatus status, Byte checkStep, Integer siteId, Integer channelId, Integer userId,
	    int orderBy, int pageNo, int pageSize) {
    ...
    public Pagination getPageByRight(String title, Integer typeId, Integer currUserId, Integer inputUserId,
	    boolean topLevel, boolean recommend, ContentStatus status, Byte checkStep, Integer siteId,
	    Integer channelId, Integer departId, Integer userId, boolean selfData, int orderBy, int pageNo,
	    int pageSize) {
	Finder f = Finder.create(
		"select  bean from Content bean  left join bean.contentShareCheckSet shareCheck left join  shareCheck.channel tarChannel ");
	if (rejected == status) {
	...
	if (channelId != null) {
	    ...
	    f.append(
		    " or (shareCheck.checkStatus<>0 and shareCheck.shareValid=true and  tarChannel.lft between parent.lft and parent.rgt and tarChannel.site.id=parent.site.id and parent.id=:parentId)))");
	    f.setParam("parentId", channelId);
	    ...
	    f.append(
		    " and bean.site.id=:siteId or (shareCheck.checkStatus<>0 and shareCheck.shareValid=true and tarChannel.site.id=:siteId and bean.site.id<>:siteId))");
	    f.setParam("siteId", siteId);
	} else {
	    if (departId != null) {
		f.append(" join bean.channel channel left join channel.departments depart");
	...
	Finder f = Finder.create("select bean from Content bean join bean.collectUsers user where user.id=:userId")
	...
	Finder f = Finder.create(hql).setParam("status", ContentCheck.CHECKED).setParam("topLevel", topLevel)
	...
	Finder f = Finder.create(hql).setParam("status", ContentCheck.CHECKED).setParam("pigeonholeDate",
		pigeonholeDay);
	return find(f);
    }

    private void appendQuery(Finder f, String title, Integer typeId, Integer inputUserId, ContentStatus status,
    ...
    public Pagination getPageBySiteIdsForTag(Integer[] siteIds, Integer[] typeIds, Boolean titleImg, Boolean recommend,
	    String title, int open, Map<String, String[]> attr, int orderBy, int pageNo, int pageSize) {
	Finder f = bySiteIds(siteIds, typeIds, titleImg, recommend, title, open, attr, orderBy);
	f.setCacheable(true);
	return find(f, pageNo, pageSize);
    }
    ...
    public Pagination getPageByChannelIdsForTag(Integer[] channelIds, Integer[] typeIds, Boolean titleImg,
	    Boolean recommend, String title, int open, Map<String, String[]> attr, int orderBy, int option, int pageNo,
	    int pageSize) {
	Finder f = byChannelIds(channelIds, typeIds, titleImg, recommend, title, open, attr, orderBy, option);
	f.setCacheable(true);
	return find(f, pageNo, pageSize);
    }

    @SuppressWarnings("unchecked")
    public List<Content> getListByChannelIdsForTag(Integer[] channelIds, Integer[] typeIds, Boolean titleImg,
	    Boolean recommend, String title, int open, Map<String, String[]> attr, int orderBy, int option,
    ...
    public Pagination getPageByChannelPathsForTag(String[] paths, Integer[] siteIds, Integer[] typeIds,
	    Boolean titleImg, Boolean recommend, String title, int open, Map<String, String[]> attr, int orderBy,
	    int pageNo, int pageSize) {
	Finder f = byChannelPaths(paths, siteIds, typeIds, titleImg, recommend, title, open, attr, orderBy);
	f.setCacheable(true);
	return find(f, pageNo, pageSize);
    }

    @SuppressWarnings("unchecked")
    public List<Content> getListByChannelPathsForTag(String[] paths, Integer[] siteIds, Integer[] typeIds,
	    Boolean titleImg, Boolean recommend, String title, int open, Map<String, String[]> attr, int orderBy,
	    Integer first, Integer count) {
	Finder f = byChannelPaths(paths, siteIds, typeIds, titleImg, recommend, title, open, attr, orderBy);
	if (first != null) {
    ...
    public Pagination getPageByTopicIdForTag(Integer topicId, Integer topicChannelId, Integer[] siteIds,
	    Integer[] channelIds, Integer[] typeIds, Boolean titleImg, Boolean recommend, String title, int open,
	    Map<String, String[]> attr, int orderBy, int pageNo, int pageSize) {
	Finder f = byTopicId(topicId, topicChannelId, siteIds, channelIds, typeIds, titleImg, recommend, title, open,
		attr, orderBy);
	f.setCacheable(true);
	return find(f, pageNo, pageSize);
    }
    ...
    @SuppressWarnings("unchecked")
    public List<Content> getListByTopicIdForTag(Integer topicId, Integer topicChannelId, Integer[] siteIds,
	    Integer[] channelIds, Integer[] typeIds, Boolean titleImg, Boolean recommend, String title, int open,
	    Map<String, String[]> attr, int orderBy, Integer first, Integer count) {
	Finder f = byTopicId(topicId, topicChannelId, siteIds, channelIds, typeIds, titleImg, recommend, title, open,
		attr, orderBy);
	if (first != null) {
	    f.setFirstResult(first);
    ...
    public Pagination getPageByTagIdsForTag(Integer[] tagIds, Integer[] siteIds, Integer[] channelIds,
	    Integer[] typeIds, Integer excludeId, Boolean titleImg, Boolean recommend, String title, int open,
	    Map<String, String[]> attr, int orderBy, int pageNo, int pageSize) {
	Finder f = byTagIds(tagIds, siteIds, channelIds, typeIds, excludeId, titleImg, recommend, title, open, attr,
		orderBy);
	f.setCacheable(true);
	return find(f, pageNo, pageSize);
    }
    ...
    @SuppressWarnings("unchecked")
    public List<Content> getListByTagIdsForTag(Integer[] tagIds, Integer[] siteIds, Integer[] channelIds,
	    Integer[] typeIds, Integer excludeId, Boolean titleImg, Boolean recommend, String title, int open,
	    Map<String, String[]> attr, int orderBy, Integer first, Integer count) {
	Finder f = byTagIds(tagIds, siteIds, channelIds, typeIds, excludeId, titleImg, recommend, title, open, attr,
		orderBy);
    ...
    private Finder bySiteIds(Integer[] siteIds, Integer[] typeIds, Boolean titleImg, Boolean recommend, String title,
	    int open, Map<String, String[]> attr, int orderBy) {
	Finder f = Finder.create("select  bean from Content bean");
    ...
    private Finder byChannelIds(Integer[] channelIds, Integer[] typeIds, Boolean titleImg, Boolean recommend,
	    String title, int open, Map<String, String[]> attr, int orderBy, int option) {
	Finder f = Finder.create();
	...
	if (option == 0 || len > 1) {
	    ...
	    if (len == 1) {
		f.append(
			" where ((bean.channel.id=:channelId ) or (shareCheck.checkStatus<>0 and shareCheck.shareValid=true and  tarChannel.id=:channelId))");
		f.setParam("channelId", channelIds[0]);
	    } else {
		f.append(
			" where ((bean.channel.id in (:channelIds) ) or (shareCheck.checkStatus<>0 and shareCheck.shareValid=true and  tarChannel.id in (:channelIds)))");
	    ...
	    f.append(
		    " and parent.id=:channelId ) or (shareCheck.checkStatus<>0 and shareCheck.shareValid=true and  tarChannel.lft between parent.lft and parent.rgt and tarChannel.site.id=parent.site.id and parent.id=:channelId))");
	    f.setParam("channelId", channelIds[0]);
	} else if (option == 2) {
	    // 包含副栏目
	    f.append("select  bean from Content bean");
	    ...
	    f.append(
		    " where ((channel.id=:channelId ) or (shareCheck.checkStatus<>0 and shareCheck.shareValid=true and  tarChannel.id=:channelId))");
	    f.setParam("channelId", channelIds[0]);
	} else {
    ...
    private Finder byChannelPaths(String[] paths, Integer[] siteIds, Integer[] typeIds, Boolean titleImg,
	...
	if (len == 1) {
	    ...
	    f.append(
		    " where (channel.path in (:paths) or (shareCheck.checkStatus<>0 and shareCheck.shareValid=true and  tarChannel.path in (:paths)))");
	...
	if (siteIds != null) {
	    ...
	    if (len == 1) {
		f.append(
			" and (channel.site.id=:siteId   or (shareCheck.checkStatus<>0 and shareCheck.shareValid=true and  tarChannel.site.id=:siteId))");
		f.setParam("siteId", siteIds[0]);
	    } else if (len > 1) {
		f.append(
			" and (channel.site.id in (:siteIds)  or (shareCheck.checkStatus<>0 and shareCheck.shareValid=true and  tarChannel.site.id  in (:siteIds) ))");
    ...
    private Finder byTopicId(Integer topicId, Integer topicChannelId, Integer[] siteIds, Integer[] channelIds,
	    Integer[] typeIds, Boolean titleImg, Boolean recommend, String title, int open, Map<String, String[]> attr,
	    int orderBy) {
	Finder f = Finder.create();
	/* f.append("select bean from Content bean join bean.topics topic"); */
	f.append("select bean from Content bean join bean.contentTopic topic");
	...
	if (topicChannelId != null) {
	    f.append(" and topic.topicChannelId=:topicChannelId");
	    f.setParam("topicChannelId", topicChannelId);
	}
	if (titleImg != null) {
	    f.append(" and bean.hasTitleImg=:titleImg");
	    f.setParam("titleImg", titleImg);
    ...
    private Finder byTagIds(Integer[] tagIds, Integer[] siteIds, Integer[] channelIds, Integer[] typeIds,
	    Integer excludeId, Boolean titleImg, Boolean recommend, String title, int open, Map<String, String[]> attr,
	    int orderBy) {
	Finder f = Finder.create();
	...
	if (siteIds != null) {
	    ...
	    if (len == 1) {
		f.append(
			" and (bean.site.id=:siteId  or (shareCheck.checkStatus<>0 and shareCheck.shareValid=true and tarChannel.site.id=:siteId))");
		f.setParam("siteId", siteIds[0]);
	    } else if (len > 1) {
		f.append(
			" and (bean.site.id in (:siteIds)  or (shareCheck.checkStatus<>0 and shareCheck.shareValid=true and tarChannel.site.id in (:siteIds)))");
		f.setParamList("siteIds", siteIds);
	    }
	}
    }
	...
	if (attr != null && !attr.isEmpty()) {
		...
		if (operate.equals(PARAM_ATTR_EQ)) {
		    f.append(" and bean.attr[:k" + key + "]=:v" + key).setParam("k" + key, key).setParam("v" + key,
			    value);
		} else if (operate.equals(PARAM_ATTR_START)) {
		    f.append(" and bean.attr[:k" + key + "] like :v" + key).setParam("k" + key, key).setParam("v" + key,
			    value + "%");
			...
			if (floatValue >= 0) {
			    f.append(" and (bean.attr[:k" + key + "]>=0 and abs(bean.attr[:k" + key + "])>:v" + key
				    + ")").setParam("k" + key, key).setParam("v" + key, floatValue);
			} else {
			    f.append(" and ((bean.attr[:k" + key + "]<0 and abs(bean.attr[:k" + key + "])<:v" + key
				    + ") or bean.attr[:k" + key + "]>=0)").setParam("k" + key, key)
				    .setParam("v" + key, -floatValue);
			}
		    } else if (operate.equals(PARAM_ATTR_GTE)) {
			if (floatValue >= 0) {
			    f.append(" and (abs(bean.attr[:k" + key + "])>=:v" + key + " and bean.attr[:k" + key
				    + "]>=0)").setParam("k" + key, key).setParam("v" + key, floatValue);
			} else {
			    f.append(" and ((abs(bean.attr[:k" + key + "])<=:v" + key + " and bean.attr[:k" + key
				    + "]<0) or bean.attr[:k" + key + "]>=0)").setParam("k" + key, key)
				    .setParam("v" + key, -floatValue);
			}
     ...
     * pageSize) { int totalCount = countQueryResult(finder);
     * System.out.println("totalCount="+totalCount); Pagination p = new
