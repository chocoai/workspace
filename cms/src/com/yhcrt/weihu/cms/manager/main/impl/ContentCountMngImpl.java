...
import com.yhcrt.weihu.cms.dao.main.ContentCountDao;
...
public class ContentCountMngImpl implements ContentCountMng {
	...
	public void commentMinusCount(Integer contentId) {
		short zero = 0;
		ContentCount contentCount= findById(contentId);
		if(contentCount.getComments() != null && contentCount.getComments() > 0)
    		contentCount.setComments(contentCount.getComments() - 1);
    		else contentCount.setComments(0);
    		
    		if(contentCount.getCommentsMonth() != null&& contentCount.getCommentsMonth() > 0)
        		contentCount.setCommentsMonth(contentCount.getCommentsMonth() - 1);
        	else contentCount.setCommentsMonth(0);
    		
    		if(contentCount.getCommentsWeek() != null && contentCount.getCommentsWeek() > zero)
    			contentCount.setCommentsWeek((short) (contentCount.getCommentsWeek() - 1));
        	else contentCount.setComments(0);
    		
    		if(contentCount.getCommentsDay() != null && contentCount.getCommentsDay() > zero)
    		contentCount.setCommentsDay((short) (contentCount.getCommentsDay() - 1));
    		else contentCount.setCommentsDay(zero);
	}
