...
import com.yhcrt.weihu.cms.dao.assist.CmsSiteAccessPagesDao;
 ...
 */
@Repository
public class CmsSiteAccessPagesDaoImpl extends HibernateBaseDao<CmsSiteAccessPages, Integer>
	implements CmsSiteAccessPagesDao {

    @SuppressWarnings("unchecked")
    public CmsSiteAccessPages findAccessPage(String sessionId, Integer pageIndex) {
	Finder f = Finder
		.create("from CmsSiteAccessPages bean where bean.sessionId=:sessionId and bean.pageIndex=:pageIndex")
		.setParam("sessionId", sessionId).setParam("pageIndex", pageIndex);
	...
	Finder f = Finder
		.create("select bean.accessPage,count(bean.accessPage),count(distinct bean.sessionId),sum(bean.visitSecond)/count(bean.accessPage) "
			+ "from CmsSiteAccessPages bean where bean.site.id=:siteId")
