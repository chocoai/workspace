package com.yhcrt.weihu.cms.action;
...
import com.yhcrt.weihu.common.image.ImageScale;
...
import com.yhcrt.weihu.core.web.WebCoreErrors;
...
public class CommonUpload {
    ...
    protected void swfPicsUpload(String root, Integer uploadNum,
	    @RequestParam(value = "Filedata", required = false) MultipartFile file, HttpServletRequest request,
	...
	if (errors.hasErrors()) {
	    ...
	    cmsUserMng.updateUploadSize(CmsUtils.getUserId(request),
    ...
    protected void swfAttachsUpload(String root, Integer uploadNum,
	    @RequestParam(value = "Filedata", required = false) MultipartFile file, HttpServletRequest request,
	    HttpServletResponse response, ModelMap model) throws Exception {
	JSONObject data = new JSONObject();
     ...
     * @return
     */
    protected WebCoreErrors validateImage(MultipartFile file, HttpServletRequest request) {
     ...
     * @return
     */
    protected WebErrors validateUpload(MultipartFile file, HttpServletRequest request) {
	String origName = file.getOriginalFilename();
	CmsUser user = CmsUtils.getUser(request);
	String ext = FilenameUtils.getExtension(origName).toLowerCase(Locale.ENGLISH);
    ...
    protected WebErrors validateFile(MultipartFile file, CmsUser user, HttpServletRequest request) {
	String origName = file.getOriginalFilename();
	...
	if (imgMark) {
	    ...
	    imageScale.imageMark(tempFile, tempFile, conf.getMinWidth(), conf.getMinHeight(), conf.getPos(),
		    conf.getOffsetX(), conf.getOffsetY(), markImg);
	} else {
	    imageScale.imageMark(tempFile, tempFile, conf.getMinWidth(), conf.getMinHeight(), conf.getPos(),
		    conf.getOffsetX(), conf.getOffsetY(), conf.getContent(), Color.decode(conf.getColor()),
		    conf.getSize(), conf.getAlpha());
	}
	return tempFile;
    }
