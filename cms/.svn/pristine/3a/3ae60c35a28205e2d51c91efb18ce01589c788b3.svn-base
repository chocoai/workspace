...
import static com.yhcrt.weihu.cms.entity.main.ContentCheck.DRAFT;
...
import com.yhcrt.weihu.cms.entity.main.Channel.AfterCheckEnum;
...
/*
 * import com.yhcrt.weihu.cms.manager.main.CmsTopicChannelMng; import
 * com.yhcrt.weihu.cms.manager.main.CmsTopicMng;
 */
import com.yhcrt.weihu.cms.manager.main.ContentCheckMng;
import com.yhcrt.weihu.cms.manager.main.ContentCountMng;
import com.yhcrt.weihu.cms.manager.main.ContentDocMng;
import com.yhcrt.weihu.cms.manager.main.ContentExtMng;
import com.yhcrt.weihu.cms.manager.main.ContentMng;
import com.yhcrt.weihu.cms.manager.main.ContentShareCheckMng;
import com.yhcrt.weihu.cms.manager.main.ContentTagMng;
import com.yhcrt.weihu.cms.manager.main.ContentTxtMng;
import com.yhcrt.weihu.cms.manager.main.ContentTypeMng;
import com.yhcrt.weihu.cms.service.ChannelDeleteChecker;
import com.yhcrt.weihu.cms.service.ContentListener;
import com.yhcrt.weihu.cms.staticpage.StaticPageSvc;
...
import com.yhcrt.weihu.core.manager.CmsWorkflowEventMng;
...
public class ContentMngImpl implements ContentMng, ChannelDeleteChecker {
	...
	if (allChannel && selfData) {
	    // 拥有所有栏目权限，只能管理自己的数据
	    p = dao.getPageBySelf(title, typeId, inputUserId, topLevel, recommend, status, checkStep, siteId, channelId,
		    userId, orderBy, pageNo, pageSize);
	} else if (allChannel && !selfData) {
	    // 拥有所有栏目权限，能够管理不属于自己的数据
	    p = dao.getPage(title, typeId, currUserId, inputUserId, topLevel, recommend, status, checkStep, siteId,
		    null, channelId, orderBy, pageNo, pageSize);
	} else {
	    p = dao.getPageByRight(title, typeId, currUserId, inputUserId, topLevel, recommend, status, checkStep,
	...
	return dao.getPage(title, typeId, null, inputUserId, topLevel, recommend, status, null, siteId, null, null,
		orderBy, pageNo, pageSize);
    }

    public Pagination getPageForMember(String title, Integer channelId, Integer siteId, Integer modelId,
	    Integer memberId, int pageNo, int pageSize) {
	return dao.getPage(title, null, memberId, memberId, false, false, ContentStatus.all, null, siteId, modelId,
	...
	return dao.getPageBySiteIdsForTag(siteIds, typeIds, titleImg, recommend, title, open, attr, orderBy, pageNo,
		pageSize);
    }
    ...
    public List<Content> getListBySiteIdsForTag(Integer[] siteIds, Integer[] typeIds, Boolean titleImg,
	    ...
	    Integer count) {
	return dao.getListBySiteIdsForTag(siteIds, typeIds, titleImg, recommend, title, open, attr, orderBy, first,
		count);
    }
    ...
    public Pagination getPageByChannelIdsForTag(Integer[] channelIds, Integer[] typeIds, Boolean titleImg,
	    ...
	    int pageSize) {
	return dao.getPageByChannelIdsForTag(channelIds, typeIds, titleImg, recommend, title, open, attr, orderBy,
	...
	return dao.getListByChannelIdsForTag(channelIds, typeIds, titleImg, recommend, title, open, attr, orderBy,
		option, first, count);
    }
	...
	return dao.getPageByChannelPathsForTag(paths, siteIds, typeIds, titleImg, recommend, title, open, attr, orderBy,
		pageNo, pageSize);
    }
    ...
    public List<Content> getListByChannelPathsForTag(String[] paths, Integer[] siteIds, Integer[] typeIds,
	    Boolean titleImg, Boolean recommend, String title, int open, Map<String, String[]> attr, int orderBy,
	    Integer first, Integer count) {
	return dao.getListByChannelPathsForTag(paths, siteIds, typeIds, titleImg, recommend, title, open, attr, orderBy,
		first, count);
    }
	...
	return dao.getPageByTopicIdForTag(topicId, topicChannelId, siteIds, channelIds, typeIds, titleImg, recommend,
		title, open, attr, orderBy, pageNo, pageSize);
    }
	...
	return dao.getListByTopicIdForTag(topicId, topicChannelId, siteIds, channelIds, typeIds, titleImg, recommend,
    ...
    public Pagination getPageByTagIdsForTag(Integer[] tagIds, Integer[] siteIds, Integer[] channelIds,
	    ...
	    Map<String, String[]> attr, int orderBy, int pageNo, int pageSize) {
	return dao.getPageByTagIdsForTag(tagIds, siteIds, channelIds, typeIds, excludeId, titleImg, recommend, title,
    ...
    public List<Content> getListByTagIdsForTag(Integer[] tagIds, Integer[] siteIds, Integer[] channelIds,
	    Integer[] typeIds, Integer excludeId, Boolean titleImg, Boolean recommend, String title, int open,
	    Map<String, String[]> attr, int orderBy, Integer first, Integer count) {
	return dao.getListByTagIdsForTag(tagIds, siteIds, channelIds, typeIds, excludeId, titleImg, recommend, title,
    ...
    public Content save(Content bean, ContentExt ext, ContentTxt txt, ContentDoc doc, Integer[] channelIds,
	    Integer topicId, Integer topicChannelId, Integer[] viewGroupIds, String[] tagArr, String[] attachmentPaths,
	...
	// 保存专题
	/*
	 * if (topicIds != null && topicIds.length > 0) { for (Integer tid :
	 * topicIds) { if(tid!=null&&tid!=0){
	 * bean.addToTopics(cmsTopicMng.findById(tid)); } } }
	 */
	if (topicId != null) {
	    bean.addToContentTopic(topicId, topicChannelId);
	}
	// 保存专题子栏目
	/*
	 * if (topicChannelIds != null && topicChannelIds.length > 0) { for
	 * (Integer tid : topicChannelIds) { if (tid != null && tid != 0) {
	 * bean.addToTopicChannels(topicChannelMng.findById(tid)); } } }
	 */
    ...
    // 导入word执行
    public Content save(Content bean, ContentExt ext, ContentTxt txt, ContentDoc doc, Integer channelId, Integer typeId,
	    Boolean draft, CmsUser user, boolean forMember) {
    ...
    private Content saveContent(Content bean, ContentExt ext, ContentTxt txt, ContentDoc doc, Integer channelId,
    ...
    public Content update(Content bean, ContentExt ext, ContentTxt txt, ContentDoc doc, String[] tagArr,
	    Integer[] channelIds, Integer topicId, Integer topicChannelId, Integer[] viewGroupIds,
	    String[] attachmentPaths, String[] attachmentNames, String[] attachmentFilenames, String[] picPaths,
	    String[] picDescs, Map<String, String> attr, Integer channelId, Integer typeId, Boolean draft, CmsUser user,
	    boolean forMember) {
	...
	// 更新专题表
	/*
	 * Set<CmsTopic> topics = bean.getTopics(); topics.clear(); if (topicIds
	 * != null && topicIds.length > 0) { for (Integer tid : topicIds) {
	 * if(tid!=null&&tid!=0){ topics.add(cmsTopicMng.findById(tid)); } } }
	 */

	if (topicId != null) {
	    List<ContentTopic> contentTopics = bean.getContentTopic();
	    contentTopics.clear();
	    bean.addToContentTopic(topicId, topicChannelId);
	}
	// 更新浏览会员组
	...
	if (topicIds != null && topicIds.length > 0) {
		...
		ContentTopic contentTopic = new ContentTopic(topicId, null);
    ...
    public Content addContentToTopics(Integer contentId, Integer topicId, Integer topicChannelId) {
	Content bean = findById(contentId);
	List<ContentTopic> contentTopics = bean.getContentTopic();
	if (topicId != null) {
	    ContentTopic contentTopic = new ContentTopic(topicId, topicChannelId);
	    contentTopics.add(contentTopic);
	}
	return bean;
    }
	...
	int workflowstep = workflowMng.check(workflow, content.getUser(), user, Content.DATA_CONTENT, content.getId(),
		null);
	if (workflowstep == -1) {
	...
	if (content.getStatus().equals(ContentCheck.CHECKING) || content.getStatus().equals(ContentCheck.CHECKED)) {
	    ...
	    int workflowstep = workflowMng.reject(workflow, content.getUser(), user, Content.DATA_CONTENT,
    ...
    public Content[] contentStatic(Integer[] ids) throws TemplateNotFoundException, TemplateParseException,
    ...
    /*
     * @Autowired private CmsTopicChannelMng topicChannelMng;
     */
    ...
    /*
     * @Autowired public void setCmsTopicMng(CmsTopicMng cmsTopicMng) {
