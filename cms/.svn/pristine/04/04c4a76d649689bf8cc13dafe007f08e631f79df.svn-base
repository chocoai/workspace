package com.yhcrt.weihu.core.action.front;

import static com.yhcrt.weihu.core.manager.AuthenticationMng.AUTH_KEY;
 ...
 * <ul>
 * <li>用户信息来源。
 * 可以有多种来源，最合适的来源是LDAP，也可以是数据库。本认证中心为实现更实用于互联网的、更轻量级的单点登录，使用数据库作为用户信息来源。
 * 只要在同一数据库实例下，即可访问到用户信息。各系统可以自行实现注册、修改密码、禁用等功能。如果是用一体系下的应用，这些功能都提供了统一接口。
 * <li>用户认证信息。用户登录成功后，需要保存登录信息，最合适的保存技术是memcached，也可以是其他集群缓存或数据库。
 * 本认证中心使用数据库保存登录信息。 只要在同一数据库实例下，即可访问。各系统可以自行实现退出登录。
 * </ul>
 */
@Controller
public class LoginAct {
    ...
    public String submit(String username, String password, String processUrl, String returnUrl, String message,
	    HttpServletRequest request, HttpServletResponse response, ModelMap model) {
	WebCoreErrors errors = validateSubmit(username, password, request);
	if (!errors.hasErrors()) {
	    try {
		Authentication auth = authMng.login(username, password, RequestUtils.getIpAddr(request), request,
			response, session);
		String view = getView(processUrl, returnUrl, auth.getId());
		if (view != null) {
		    return view;
    ...
    private WebCoreErrors validateSubmit(String username, String password, HttpServletRequest request) {
