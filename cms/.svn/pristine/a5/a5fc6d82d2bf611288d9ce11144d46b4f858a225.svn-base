...
import com.yhcrt.weihu.cms.dao.assist.CmsOracleDataBackDao;
...
public class CmsOracleDataBackDaoImpl extends JdbcDaoSupport implements
	...
	private boolean isDateTimeColumn(String tablename,int columnIndex){
		if(StringUtils.isNotBlank(tablename)){
			...
			if(upperTableName.equals("JC_PLUG")){
				if(columnIndex==6||columnIndex==7||columnIndex==8){
					return true;
				}
			}
			if(upperTableName.equals("JC_SITE_ACCESS")){
				if(columnIndex==4||columnIndex==5){
					return true;
				}
			}
			if(upperTableName.equals("JC_SITE_ACCESS_PAGES")){
				if(columnIndex==4||columnIndex==5){
					return true;
				}
			}
			if(upperTableName.equals("JC_SITE_ACCESS_COUNT")){
				if(columnIndex==4){
					return true;
				}
			}
			if(upperTableName.equals("JC_SITE_ACCESS_STATISTIC")){
				if(columnIndex==2){
					return true;
				}
			}
	...
	private boolean isColbColumn(String tablename,int columnIndex){
		if(StringUtils.isNotBlank(tablename)){
			...
			if(upperTableName.equals("JC_DIRECTIVE_TPL")){
				//JOB_DESCRIPTION
				if(columnIndex==4){
					return true;
				}
