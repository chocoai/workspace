...
import com.yhcrt.weihu.cms.entity.assist.CmsAcquisition;
import com.yhcrt.weihu.cms.entity.assist.CmsTask;
import com.yhcrt.weihu.cms.entity.main.Channel;
import com.yhcrt.weihu.cms.manager.assist.CmsAcquisitionMng;
import com.yhcrt.weihu.cms.manager.assist.CmsTaskMng;
...
public class CmsTaskAct {
	...
	Pagination pagination = manager.getPage(CmsUtils.getSiteId(request), cpn(pageNo),
    ...
    public String save(CmsTask bean, HttpServletRequest request, ModelMap model)
	...
	if (bean.getType().equals(CmsTask.TASK_STATIC_INDEX)) {
	    ...
	    bean.setAttr(attrs);
	} else if (bean.getType().equals(CmsTask.TASK_STATIC_CHANNEL)
		|| bean.getType().equals(CmsTask.TASK_STATIC_CONTENT)) {
	    attrs.put(CmsTask.TASK_PARAM_SITE_ID, siteId);
	}
	bean.setAttr(attrs);
    ...
    public String update(CmsTask bean, Integer pageNo, HttpServletRequest request, ModelMap model)
	...
	if (cronExpress.indexOf("null") == -1) {
	    ...
	    CronTrigger cronTrigger = new CronTrigger("cron_" + taskCode, Scheduler.DEFAULT_GROUP, jobDetail.getName(),
